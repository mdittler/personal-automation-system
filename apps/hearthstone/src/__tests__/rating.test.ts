...
describe('buildRateButtons', () => {
	...
	it('first row contains thumbs up and thumbs down buttons', () => {
		...
		expect(callbacks).toContain('app:hearthstone:rate:up:2026-03-31');
		expect(callbacks).toContain('app:hearthstone:rate:down:2026-03-31');
	});
	...
	it('second row contains skip button', () => {
		...
		expect(row[0].callbackData).toBe('app:hearthstone:rate:skip:2026-03-31');
	});
