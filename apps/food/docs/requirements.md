...
Food is a PAS app. It must follow the PAS app contract:
...
```bash
pnpm scaffold-app --name=food --description="Household food management — recipes, meal planning, grocery lists, pantry tracking, nutrition, and family food intelligence." --author="PAS Team"
```

This creates `apps/food/` with the scaffold template. Then customize the generated `manifest.yaml` and `src/index.ts` per the specs below.
...
Declare a broad intent like "user has a food-related question" so that general food queries route to Food rather than the default chatbot.
...
Food should publish the following events for other PAS apps to consume:

| Event | Payload | Potential Consumer |
|---|---|---|
| `food.mealPlan.finalized` | Plan with dates and recipes | Calendar app (block prep time) |
| `food.groceryList.ready` | Grocery list | Reminders app (tie to errand window) |
| `food.recipe.scheduled` | Recipe + date | Reminders app (defrost reminder night before if frozen protein) |
| `food.meal.cooked` | Recipe + date + who cooked | Logging/journaling apps |
| `food.shopping.completed` | Store, total cost, items | Finance tracking apps |
...
Food uses two data scopes from PAS:

- **Shared data** (`services.data.forShared()`) — household-wide data that all linked users access. Scoped to `data/users/shared/food/`.
- **Per-user data** (`services.data.forUser(userId)`) — individual preferences, macro logs, and personal tracking. Scoped to `data/users/<userId>/food/`.

### Shared Data (household-wide)

```
data/users/shared/food/
...
**Tag taxonomy for Food:**

| Pattern | Purpose | Examples |
|---|---|---|
| `pas/food` | Source app | Always included |
...
**Wiki-links:** Recipe files should use wiki-links to reference related grocery lists and meal plans (e.g., `[[_shared/food/grocery/current]]`). This enables Obsidian graph view across the food management system.
...
The following is the target `manifest.yaml` for Food. Implement incrementally per the phased rollout — only declare capabilities that are built.
...
**NF-8. Logging.** Use `services.logger` (Pino-based, auto-tagged with `food` app ID) for all logging. Use `info` for user-facing operations, `debug` for data operations, `error` for failures. Never use `console.log`.
