 ...
 * sequences against the per-user YAML stores.
 *
 * Why this exists: ScopedDataStore.write is atomic at the filesystem level,
 * but our store helpers do `read → mutate → write`, and two async paths can
 * interleave across the awaits in a single Node process. Without serialization
 * the second writer overwrites the first writer's mutation. The fix is to
 * funnel every RMW for a given (store, key) through a single promise chain.
 *
 * Usage:
 *   const lock = new AsyncLock();
 *   await lock.run(`quick-meals:${userId}`, async () => {
