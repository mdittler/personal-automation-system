...
export async function handleFamilyCommand(
	...
	if (subcommand === 'remove') {
		...
		return {
			...
			buttons: [
				[
					{ text: '🗑 Yes, remove', callbackData: `app:food:fa:rm:${slug}` },
					{ text: '❌ Cancel', callbackData: 'app:food:fa:rm-cancel' },
				],
			],
		};
	}
...
function buildProfileEditButtons(slug: string): InlineButton[][] {
	return [
		[
			{ text: '📝 Edit stage', callbackData: `app:food:fa:es:${slug}` },
			{ text: '✅ Add safe', callbackData: `app:food:fa:as:${slug}` },
			{ text: '⚠️ Add avoid', callbackData: `app:food:fa:aa:${slug}` },
		],
	];
}
...
export async function handleFoodIntroduction(
	...
	return {
		...
		buttons: [
			[
				{ text: '😊 None', callbackData: `app:food:fi:r:${child.profile.slug}:none` },
				{ text: '🤔 Mild', callbackData: `app:food:fi:r:${child.profile.slug}:mild` },
...
export function buildRecipeApprovalButtons(
	...
	for (const child of children) {
		...
		buttons.push({
			...
			callbackData: `app:food:fa:${action}:${slug}:${recipeId}`,
		});
	}
