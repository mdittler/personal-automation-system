		...
		if (data === 'clear') {
			...
			if (purchased.length > 0) {
				...
				await services.telegram.editMessage(
					...
					[
						[
							{ text: '📦 Add all to pantry', callbackData: 'app:food:pantry-all' },
							{ text: '⏭ Skip', callbackData: 'app:food:pantry-skip' },
						],
					],
				);
...
async function handleWhatsForDinner(ctx: MessageContext): Promise<void> {
	...
	const buttons = [
		[
			{ text: '📖 Full Recipe', callbackData: `app:food:show-recipe:${today}` },
			{ text: '🔄 Swap', callbackData: `app:food:swap:${today}` },
		],
	];
