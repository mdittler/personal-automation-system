...
describe('FoodShadowLogger', () => {
	...
	it('formats every field correctly for a text "ok" entry', async () => {
		...
		expect(c).toMatch(
			/- \*\*Core route\*\*: food \/ user wants to add items to the grocery list \(confidence: 0\.88, source: intent, verifier: agreed\)/,
		);
		expect(c).toMatch(
			/- \*\*Regex winner\*\*: grocery_add → "user wants to add items to the grocery list"/,
		);
		expect(c).toMatch(
			/- \*\*Shadow\*\*: \{"action":"user wants to add items to the grocery list","confidence":0\.94\}/,
		);
	...
	it('renders all ShadowResult kinds correctly', async () => {
		...
		await logger.log(
			sampleEntry({
				shadow: { kind: 'skipped-pending-flow', flow: 'cook_recipe' },
				verdict: 'skipped',
			}),
		);
		await logger.log(sampleEntry({ shadow: { kind: 'skipped-cook-mode' }, verdict: 'skipped' }));
		await logger.log(
			sampleEntry({ shadow: { kind: 'skipped-number-select' }, verdict: 'skipped' }),
		);
		await logger.log(
			sampleEntry({ shadow: { kind: 'legacy-skipped' }, verdict: 'legacy-skipped' }),
		);
		await logger.log(
			sampleEntry({ shadow: { kind: 'parse-failed', raw: 'not json' }, verdict: 'error' }),
		);
		await logger.log(
			sampleEntry({ shadow: { kind: 'llm-error', category: 'cost-cap' }, verdict: 'skipped' }),
		);
		const c = readFileSync(join(dir, 'shadow-classifier-log.md'), 'utf8');
	...
	it('ok action with embedded quote renders as valid JSON on the Shadow line', async () => {
		...
		await logger.log(
			sampleEntry({ shadow: { kind: 'ok', action: 'user says "hi"', confidence: 0.9 } }),
		);
		const c = readFileSync(join(dir, 'shadow-classifier-log.md'), 'utf8');
