		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isSaveRecipeIntent requires save/add/store/keep+recipe keyword; "find" triggers isSearchRecipeIntent instead',
			...
			},
			{
				...
				correctLabel: 'user wants to search for a recipe',
				reason:
					'isSearchRecipeIntent /\\bwhat.*recipes?\\b.*\\bhave\\b/ matches before save-intent check',
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isWhatCanIMakeIntent /\\bwhat\\s+can\\s+i\\s+(make|cook)\\b/ fires at line 459; ingredient-constrained lookup, not open recipe search',
			...
			},
			{
				...
				reason:
					'isSaveRecipeIntent (line 417) runs before isSearchRecipeIntent (line 423); save+recipe routes to save intent',
		...
		deterministicRejectFor: [
			{
				...
				reason:
					"isWhatsForDinnerIntent (line 453) matches /what's for dinner/ — single-meal tonight query, not week planning",
			...
			},
			{
				...
				correctLabel: 'user wants to know what they can make with what they have',
				reason:
					'isWhatCanIMakeIntent /\\bwhat\\s+can\\s+i\\s+(make|cook)\\b/ fires at line 459 — ingredient-constrained cooking idea lookup, not multi-day meal planning',
				source: 'apps/food/src/index.ts:459',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isGroceryAddIntent /\\b(add)\\b.*\\b(to)\\s+(the\\s+)?(grocery)\\b/ — adding items routes to add intent, not view',
			...
			},
			{
				...
				correctLabel: 'user wants to add items to the grocery list',
				reason:
					'isGroceryAddIntent /\\bwe need\\b/ — "we need" is an add-intent trigger, not a view request',
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isGroceryViewIntent /\\b(show|view|see|check)\\b.*\\b(grocery|shopping)\\b/ — viewing, not adding',
			...
			},
			{
				...
				reason:
					'isPantryRemoveIntent /\\bwe\'re out of\\b/ fires at line 694 — "out of" is a pantry-remove signal that maps to pantry label in REGEX_TO_MANIFEST_MAP, not grocery-add',
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isMealPlanGenerateIntent /\\b(plan|generate|create|make)\\b.*\\b(meals?)\\b/ — week-level planning, not tonight query',
			...
			},
			{
				...
				correctLabel: 'user wants to know what they can make with what they have',
				reason:
					'isWhatCanIMakeIntent /\\bwhat\\s+can\\s+i\\s+(make|cook)\\b/ fires — ingredient-constrained query captures this phrase before chatbot fallback',
				source: 'apps/food/src/index.ts:459',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isLogMealNLIntent /^i (had|ate)/ (line 578) runs before food-question intent (line 664) — "I had" always triggers meal-log flow',
			...
			},
			{
				...
				reason:
					'isBudgetViewIntent (line 650) runs before isFoodQuestionIntent (line 664); spending queries are a separate intent',
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to search for a recipe',
				reason:
					'isSearchRecipeIntent (line 423) matches "find...recipe" — searching for a recipe, not starting a cook session on a known recipe',
			...
			},
			{
				...
				correctLabel: 'user wants to plan meals for the week',
				reason:
					'isMealPlanGenerateIntent /\\b(make)\\b.*\\b(meals?)\\b/ fires at line 441 before isCookIntent at line 706 — week-planning intent captures "make...meals" before cook intent runs',
				source: 'apps/food/src/index.ts:441',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to add items to the grocery list',
				reason:
					'isGroceryAddIntent /\\b(add)\\b.*\\b(to)\\s+(the\\s+)?(grocery)\\b/ — explicit "grocery list" routes to grocery-add, not pantry',
				source: 'apps/food/src/index.ts:3181',
			},
		],
		advisoryNearMisses: [
			{
				...
				correctLabel: 'user wants to add items to the grocery list',
				reason:
					'isGroceryAddIntent /\\b(add|put|get|buy)\\b.*\\b(to|on)\\s+(the\\s+)?(grocery|shopping)\\b/ may not match "the list" without grocery/shopping keyword; falls to LLM routing which typically classifies as grocery-add',
				source: 'apps/food/src/index.ts:3181',
			},
		],
	},
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to log a meal they cooked by name with an optional portion',
				reason:
					'isLogMealNLIntent /^i (had|ate)/ (line 578) fires — "I had leftover X" is classified as a meal-log (eating leftovers), not logging that leftovers exist in the fridge',
				source: 'apps/food/src/index.ts:578',
			},
			{
				...
				correctLabel: 'user wants to know what they can make with what they have',
				reason:
					'isWhatCanIMakeIntent /\\bwhat\\s+can\\s+i\\s+(make|cook)\\b/ fires at line 459 — ingredient-constrained recipe lookup, not logging that leftovers exist in the fridge',
				source: 'apps/food/src/index.ts:459',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isCulturalCalendarIntent (line 630) runs before isHostingIntent (line 636) and matches holiday/occasion keywords; Christmas triggers cultural calendar intent',
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to see or modify the grocery list',
				reason:
					'isGroceryViewIntent (line 676) fires on "show...grocery list" — viewing the list, not spending data',
		...
		advisoryNearMisses: [
			{
				...
				reason:
					'store-price query — may route via isPriceUpdateIntent (line 567) or LLM routing; aggregate budget view vs per-store pricing is often LLM-decided for ambiguous phrasing',
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to see food spending',
				reason:
					'aggregate spending query routes to budget view; receipt lookup is for a specific receipt document, not total spend',
				source: 'apps/food/src/index.ts:650',
			},
			{
				...
				correctLabel: 'user asks about prices at a specific store',
				reason:
					'isPriceUpdateIntent (line 567) matches store + price queries; per-item price lookup is a different intent from receipt document lookup',
				source: 'apps/food/src/index.ts:567',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to see food spending',
				reason:
					'isBudgetViewIntent (line 650) matches aggregate spending queries; per-item store prices are a different intent',
				source: 'apps/food/src/index.ts:650',
			},
			{
				...
				reason:
					'viewing a receipt document is distinct from asking about current store prices; receipt intent handles document lookups',
			},
		],
		advisoryNearMisses: [
			{
				...
				correctLabel: 'user asks about prices at a specific store',
				reason:
					'isPriceUpdateIntent (line 566) saves a new price — same shadow label as price-query intent but write semantics; accept array should reflect query phrasings, not update statements',
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isAdherenceIntent (line 598) matches "how am I doing on macros" and is explicitly excluded from isNutritionViewIntent — adherence check runs first',
			...
			},
			{
				...
				correctLabel: 'user wants to log a meal they cooked by name with an optional portion',
				reason:
					'isLogMealNLIntent (line 578) runs before isNutritionViewIntent (line 615); "I had" always routes to meal-log, not nutrition view',
				source: 'apps/food/src/index.ts:578',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: "user wants to know what's for dinner",
				reason:
					"isWhatsForDinnerIntent (line 453) matches /what's for dinner/ — consulting the meal plan, not asking what ingredients are available",
				source: 'apps/food/src/index.ts:453',
			},
			{
				...
				correctLabel: 'user wants to search for a recipe',
				reason:
					'isSearchRecipeIntent (line 423) matches "find...recipe" — open recipe search by type, not constrained by available ingredients',
				source: 'apps/food/src/index.ts:423',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isFoodIntroIntent (line 531) matches child name + first-try food phrasing — introducing a new food, not adapting a recipe',
			...
			},
			{
				...
				reason:
					'isChildApprovalIntent (line 549) matches child name + reaction verb — approval/rejection tag, not recipe adaptation',
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to tag a recipe as kid-approved or rejected',
				reason:
					'isChildApprovalIntent (line 549) matches child name + loved/hated/liked a food — recipe rating, not a first-time introduction',
				source: 'apps/food/src/index.ts:549',
			},
			{
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to log a new food introduction for a child',
				reason:
					'isFoodIntroIntent (line 531) matches child name + first-try food phrasing (tried) — food introduction, not recipe approval',
				source: 'apps/food/src/index.ts:531',
			},
			{
				...
				correctLabel: 'user wants to adapt a recipe for a child',
				reason:
					'isKidAdaptIntent (line 505) matches child name + recipe modification — adapting the recipe, not rating it',
				source: 'apps/food/src/index.ts:505',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				reason:
					"isLeftoverAddIntent /\\b(leftover|left over)\\b.*\\b(have|got|save|store|put|log)\\b/ or /\\b(there's|we've got)\\b.*\\bleftover\\b/ — logging food in the fridge, not recording what was eaten",
			...
			},
			{
				...
				reason:
					'isNutritionViewIntent (line 615) matches calorie/nutrition queries; isLogMealNLIntent exclusion regex filters out "how many" phrasings',
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to log a meal they cooked by name with an optional portion',
				reason:
					'isLogMealNLIntent (line 578) matches "I had" + named food — pasta is a recognizable meal, routes to named-meal-log, not unfamiliar-meal',
				source: 'apps/food/src/index.ts:578',
			},
			{
				...
				correctLabel: 'user wants to save a recipe',
				reason:
					'isSaveRecipeIntent (line 417) fires on save+recipe keyword — saving a recipe is distinct from logging an unidentified meal that was eaten',
				source: 'apps/food/src/index.ts:417',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to save a recipe',
				reason:
					'isSaveRecipeIntent (line 417) matches save+recipe — storing a recipe in the recipe collection, not as a quick-meal shortcut',
				source: 'apps/food/src/index.ts:417',
			},
			{
				text: 'I had pasta for lunch',
				correctLabel: 'user wants to log a meal they cooked by name with an optional portion',
				reason:
					'isLogMealNLIntent (line 578) fires on "I had X" — reporting a past meal, not creating a reusable quick-meal template',
				source: 'apps/food/src/index.ts:578',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to see how well they are hitting their macro targets over time',
				reason:
					'isAdherenceIntent (line 598) matches "how am I doing on macros" — checking adherence, not changing targets',
				source: 'apps/food/src/index.ts:598',
			},
			{
				...
				correctLabel: 'user wants to see nutrition information',
				reason:
					'isNutritionViewIntent (line 615) matches nutrition-view queries; "show" without change/set verb routes to view, not set',
				source: 'apps/food/src/index.ts:615',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				reason:
					'isTargetsSetIntent (line 592) matches change/set + calorie/macro targets — modifying goals, not checking adherence',
			...
			},
			{
				...
				correctLabel: 'user wants to see nutrition information',
				reason:
					"isNutritionViewIntent (line 615) matches today's nutrition summary — viewing logged data, not evaluating goal adherence over time",
				source: 'apps/food/src/index.ts:615',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to see how well they are hitting their macro targets over time',
				reason:
					'isAdherenceIntent (line 598) fires — progress-toward-goals query, not a diet-health correlation question',
				source: 'apps/food/src/index.ts:598',
			},
			{
				...
				correctLabel: 'user wants to see nutrition information',
				reason:
					'isNutritionViewIntent (line 615) handles nutrition data retrieval; health-correlation is a different, higher-level analytical intent',
				source: 'apps/food/src/index.ts:615',
			},
		],
		advisoryNearMisses: [],
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to plan meals for the week',
				reason:
					'isMealPlanGenerateIntent (line 441) matches general week planning — no holiday/cultural context signals present',
				source: 'apps/food/src/index.ts:441',
			},
		],
		advisoryNearMisses: [
		...
		deterministicRejectFor: [
			{
				...
				correctLabel: 'user wants to check or update the pantry',
				reason:
					"isPantryRemoveIntent /\\bwe're out of\\b/ (line 694) fires — looks like a general statement but is a pantry-remove intent trigger",
				source: 'apps/food/src/index.ts:694',
			},
			{
				...
				correctLabel: 'user wants to tag a recipe as kid-approved or rejected',
				reason:
					'isChildApprovalIntent (line 549) catches child name + loved/liked verb — appears to be a general observation but triggers kid-approval routing',
				source: 'apps/food/src/index.ts:549',
			},
		],
		advisoryNearMisses: [],
