...
describe('buildLabelsFromManifest', () => {
	it('returns manifest intents + "none", no duplicates', () => {
		const intents = ['user wants to save a recipe', 'user wants to search for a recipe'];
...
describe('REGEX_TO_MANIFEST_MAP', () => {
	...
	it.each([
		...
		['price_lookup', 'user asks about prices at a specific store'],
...
describe('INTENTIONALLY_UNMAPPED_LABELS', () => {
	...
	it('is exactly the two LLM-only orphan intents (snapshot)', () => {
		expect(INTENTIONALLY_UNMAPPED_LABELS).toContain(
...
describe('label categorization (Chunk D)', () => {
	...
	it('SHADOW_LABELS_WITHOUT_TEXT_HANDLER is empty because all current labels have text handlers or mappings', () => {
		expect(SHADOW_LABELS_WITHOUT_TEXT_HANDLER).toEqual([]);
	});
});
