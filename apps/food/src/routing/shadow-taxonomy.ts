...
/**
 * Labels that represent intents with NO text-side handler.
 * When the shadow classifier returns one of these for a text message, the
 * dispatcher falls through to the regex cascade.
 * Parity enforced by shadow-taxonomy.test.ts 'label categorization (Chunk D)'.
 */
export const SHADOW_LABELS_WITHOUT_TEXT_HANDLER: readonly FoodShadowLabel[] = [] as const;
...
export function normalizeRegexLabel(regexLabel: string): FoodShadowLabel {
	return (REGEX_TO_MANIFEST_MAP as Record<string, FoodShadowLabel>)[regexLabel] ?? 'none';
}
