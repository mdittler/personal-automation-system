...
export class FoodShadowLogger {
	...
	private fmtShadow(s: ShadowResult): string {
		switch (s.kind) {
			case 'ok':
				return JSON.stringify({ action: s.action, confidence: s.confidence });
			case 'skipped-sample':
				return 'skipped-sample';
			case 'skipped-no-caption':
				return 'skipped-no-caption';
			case 'skipped-pending-flow':
				return `skipped-pending-flow:${s.flow}`;
			case 'skipped-cook-mode':
				return 'skipped-cook-mode';
			case 'skipped-number-select':
				return 'skipped-number-select';
			case 'legacy-skipped':
				return 'legacy-skipped';
			case 'parse-failed':
				return `parse-failed (raw: "${safeForLog(s.raw, MAX_RAW)}")`;
			case 'llm-error':
				return `llm-error:${s.category}`;
		}
	}
}
