...
export async function saveFreezer(store: ScopedDataStore, items: FreezerItem[]): Promise<void> {
	const fm = generateFrontmatter({
		...
		date: isoNow(),
		tags: buildAppTags('food', 'freezer'),
		app: 'food',
	});
...
export function buildFreezerButtons(
	...
	rows.push([{ text: '➕ Add to Freezer', callbackData: 'app:food:fz:add' }]);
	...
	for (let i = 0; i < items.length; i++) {
		...
		rows.push([
			{ text: `🫧 Thaw: ${item.name}`, callbackData: `app:food:fz:thaw:${i}:${enc}` },
			{ text: `🗑️ Toss: ${item.name}`, callbackData: `app:food:fz:toss:${i}:${enc}` },
		]);
	}
