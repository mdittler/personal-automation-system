 ...
 *
 * Callback pattern: app:food:cooked:<date>
 */
	...
	return uncookedMeals.map((m) => [
		{
			...
			callbackData: `app:food:cooked:${m.date}`,
		},
	]);
}
 ...
 * Row 1: skip
 *
 * Callback pattern: app:food:rate:<direction>:<date>
 */
export function buildRateButtons(mealDate: string): InlineButton[][] {
	return [
		[
			{ text: '👍 Yes!', callbackData: `app:food:rate:up:${mealDate}` },
			{ text: '👎 Not great', callbackData: `app:food:rate:down:${mealDate}` },
		],
