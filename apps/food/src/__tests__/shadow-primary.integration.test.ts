 ...
 *   6. Receipt-details labels dispatch to their text handler.
 *
...
import { __resetQuickMealFlowForTests, beginQuickMealAdd } from '../handlers/quick-meal-flow.js';
import {
...
function makeStubClassifier(
	result: ShadowResult,
): ShadowClassifierInterface & { callCount: number } {
	const stub = {
		callCount: 0,
...
describe('shadow-primary router integration (Chunk D)', () => {
	...
	it('(a) high-confidence shadow dispatches; regexWinner=(shadow-dispatched)', async () => {
		...
		expect(
			addedCall,
			'handleGroceryAdd must have sent "Added N item(s)" confirmation',
		).toBeDefined();
	...
	it('(a2) macro-adherence shadow dispatch reads targets from the user store after the shared household guard', async () => {
		...
		await handleMessage(
			createTestMessageContext({ userId: 'user1', text: 'am i hitting my targets lately?' }),
		);
		await __flushShadowForTests();
	...
	it('(k) pending leftover-add preempts shadow; classify not called', async () => {
		...
		await handleMessage(
			createTestMessageContext({ userId: 'user1', text: 'chili about 2 servings' }),
		);
		await __flushShadowForTests();
	...
	it('(n) unmapped unfamiliar-meal label → nearest handler dispatched', async () => {
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: 'had some green gunk yesterday',
			}),
		);
		await __flushShadowForTests();
	...
	it('(o) unmapped quick-meal-template label → beginQuickMealAdd dispatched', async () => {
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: 'save this as a quick meal',
			}),
		);
		await __flushShadowForTests();
		...
		const quickMealCall = sendCalls.find(
			([, msg]) => msg.includes('New quick-meal') || msg.includes('Step 1'),
		);
		expect(quickMealCall, 'beginQuickMealAdd step-1 message must have been sent').toBeDefined();
	});

	// =========================================================================
	// (p) Receipt-details label dispatches to text handler
	// =========================================================================

	it('(p) receipt-details label dispatches; verdict=shadow-dispatched', async () => {
		vi.mocked(store.read).mockImplementation(async (path: string) => {
			if (path === 'household.yaml') return stringify(sampleHousehold);
			return '';
		});
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: 'show me the receipt from yesterday please',
			}),
		);
		await __flushShadowForTests();
		...
		const sends = vi.mocked(services.telegram.send).mock.calls as [string, string][];
		expect(
			sends.some(([, msg]) => msg.includes('I do not have any grocery receipts saved yet.')),
		).toBe(true);
	...
	it('(q) meal-plan view sub-intent: handleMealPlanView fires, not generate', async () => {
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: "show me this week's meal plan",
			}),
		);
		await __flushShadowForTests();
		...
		expect(vi.mocked(services.telegram.sendWithButtons)).toHaveBeenCalled();
		const buttonCalls = vi.mocked(services.telegram.sendWithButtons).mock.calls as [
			string,
			string,
			// biome-ignore lint/suspicious/noExplicitAny: test mock type cast
			any,
	...
	it('(s1) pantry bucket: "what\'s in the freezer" → handleFreezerView fires, not handlePantryView', async () => {
		...
		await handleMessage(
			createTestMessageContext({ userId: 'user1', text: "what's in the freezer" }),
		);
		await __flushShadowForTests();
		...
		expect(
			freezerEmptyCall,
			'handleFreezerView must have sent "🧊 Your freezer is empty"',
		).toBeDefined();
	...
	it('(s2) pantry bucket: "add soup to the freezer" → handleFreezerAddIntent fires, not handlePantryView', async () => {
		...
		await handleMessage(
			createTestMessageContext({ userId: 'user1', text: 'add soup to the freezer' }),
		);
		await __flushShadowForTests();
		...
		expect(
			freezerAddCall,
			'handleFreezerAddIntent must have sent "🧊 Added to freezer"',
		).toBeDefined();
		const pantryCall = sends.find(([, msg]) => msg.includes('📦 Your pantry is empty'));
		expect(pantryCall, 'handlePantryView must NOT fire').toBeUndefined();
	});
	...
	it('(s3) leftovers bucket: "show me my leftovers" → handleLeftoversView fires, not handleLeftoverAddIntent', async () => {
		...
		const stub = makeStubClassifier({
			kind: 'ok',
			action: 'user wants to log leftovers',
			confidence: 0.9,
		});
		__setShadowDepsForTests(stub, captureLogger);

		await handleMessage(
			createTestMessageContext({ userId: 'user1', text: 'show me my leftovers' }),
		);
		await __flushShadowForTests();
	...
	it('(s4) leftovers bucket: "the milk went bad" → handleWasteIntent fires, not handleLeftoverAddIntent', async () => {
		...
		await handleMessage(
			createTestMessageContext({ userId: 'user1', text: 'the milk went bad, toss it' }),
		);
		await __flushShadowForTests();
		...
		const noLeftoversCall = sends.find(([, msg]) =>
			msg.includes('🥘 You have no leftovers tracked'),
		);
	...
	it('(s4b) leftovers bucket fallback: unmatched leftover phrase routes to leftover view', async () => {
		...
		await handleMessage(
			createTestMessageContext({ userId: 'user1', text: 'store this chili for later' }),
		);
		await __flushShadowForTests();
	...
	it('(s5) grocery-list bucket: "generate a grocery list for this week" → handleGroceryGenerate fires, not handleGroceryView', async () => {
		...
		const stub = makeStubClassifier({
			kind: 'ok',
			action: 'user wants to see or modify the grocery list',
			confidence: 0.9,
		});
		__setShadowDepsForTests(stub, captureLogger);
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: 'generate a grocery list for this week',
			}),
		);
		await __flushShadowForTests();
	...
	it('(s6) save-recipe bucket: "edit my chicken recipe" → handleEditRecipe fires, not handleSaveRecipe', async () => {
		...
		const stub = makeStubClassifier({
			kind: 'ok',
			action: 'user wants to save a recipe',
			confidence: 0.9,
		});
		__setShadowDepsForTests(stub, captureLogger);
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: 'edit my chicken recipe to add more garlic',
			}),
		);
		await __flushShadowForTests();
	...
	it('(s7) search-recipe bucket: "show me the photo of that pasta recipe" → handleRecipePhotoRetrieval fires, not handleSearchRecipe', async () => {
		...
		const stub = makeStubClassifier({
			kind: 'ok',
			action: 'user wants to search for a recipe',
			confidence: 0.9,
		});
		__setShadowDepsForTests(stub, captureLogger);
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: 'show me the photo of that pasta recipe',
			}),
		);
		await __flushShadowForTests();
	...
	it('(s8) meal-plan bucket: "swap Tuesday\'s dinner" → handleMealSwap fires (not handleMealPlanGenerate)', async () => {
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: "swap Tuesday's dinner for something lighter",
			}),
		);
		await __flushShadowForTests();
	...
	it('(s9) meal-plan precedence: "generate a meal plan" → handleMealPlanGenerate fires, not handleMealPlanView', async () => {
		...
		await handleMessage(
			createTestMessageContext({
				userId: 'user1',
				text: 'generate a meal plan for this week',
			}),
		);
		await __flushShadowForTests();
		...
		expect(
			viewNoplanCall,
			'handleMealPlanView must NOT fire for a generate phrase',
		).toBeUndefined();
	});
