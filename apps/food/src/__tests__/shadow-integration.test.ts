...
describe('Shadow classifier integration (Chunk C)', () => {
	...
	it('agree: grocery_view regex fires and shadow agrees', async () => {
		const stubClassifier = makeStubClassifier({
			kind: 'ok',
			action: GROCERY_VIEW_LABEL,
			confidence: 0.95,
		});
		...
		expect(
			sends.some(([, msg]) => !msg.startsWith(HELP_MSG)),
			'grocery view handler must fire',
		).toBe(true);
	...
	it('data_query_fallback: DataQuery returns result → regexWinner set correctly', async () => {
		...
		const mockDataQuery = {
			query: vi.fn().mockResolvedValue({
				...
				files: [
					{
						path: 'users/user1/food/recipes.md',
						appId: 'food',
						type: 'recipe',
						title: 'Recipes',
						content: 'Taco Tuesday recipe.',
					},
				],
			}),
		};
		...
		(services as any).interactionContext = {
			...
			getRecent: vi
				.fn()
				.mockReturnValue([{ appId: 'food', action: 'view', timestamp: Date.now() }]),
		};
		...
		const ctx = createTestMessageContext({
			userId: 'user1',
			text: 'blorp bleep data query test xzxz',
		});
		await handleMessage(ctx);
		await __flushShadowForTests();
