...
describe('buildPlanButtons', () => {
	...
	it('includes Grocery List button', () => {
		...
		expect(groceryBtn?.callbackData).toContain('food');
	...
	it('includes Regenerate button', () => {
		...
		expect(regenBtn?.callbackData).toContain('food');
		expect(regenBtn?.callbackData).toContain('regenerate');
	});

	it('callback data follows app:food:action format', () => {
		const buttons = buildPlanButtons();
		for (const row of buttons) {
			for (const btn of row) {
				expect(btn.callbackData).toMatch(/^app:food:/);
			}
		}
	});
	...
	it('cooked button callbackData includes date for routing', () => {
		...
		const buttons = buildPlanButtons(plan);
		const cookedBtn = buttons.flat().find((b) => b.callbackData.startsWith('app:food:cooked:'));
		expect(cookedBtn).toBeDefined();
		expect(cookedBtn?.callbackData).toBe('app:food:cooked:2026-03-31');
	});
