...
export function registerContextRoutes(
	...
	server.get<{ Params: { userId: string } }>('/context/:userId', async (request, reply) => {
	...
	// Save entry
	server.post<{ Params: { userId: string } }>('/context/:userId', async (request, reply) => {
		const { userId } = request.params;
	...
	// Delete entry
	server.post<{ Params: { userId: string } }>('/context/:userId/delete', async (request, reply) => {
		const { userId } = request.params;
		const body = request.body as Record<string, string>;
