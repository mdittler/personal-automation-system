...
export function registerReportsApiRoute(
	...
	server.get('/reports', { preHandler: [requireScope('reports:read')] }, async (request, reply) => {
		try {
			...
			const visible = actor ? reports.filter((r) => isDeliveryVisible(actor, r.delivery)) : reports;
	...
	server.post(
		'/reports/:id/run',
		{ preHandler: [requireScope('reports:run')] },
		async (request, reply) => {
			const { id } = request.params as { id: string };
			...
			if (actor && !canRunReports(actor)) {
				return reply
					.status(403)
					.send({ ok: false, error: 'Insufficient privileges to run reports.' });
			}
	...
	server.post(
		...
		async (request, reply) => {
			...
			if (deliverActor && !canRunReports(deliverActor)) {
				return reply
					.status(403)
					.send({ ok: false, error: 'Insufficient privileges to deliver reports.' });
			}
