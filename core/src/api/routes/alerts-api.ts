...
export function registerAlertsApiRoute(
	...
	server.get('/alerts', { preHandler: [requireScope('alerts:read')] }, async (request, reply) => {
		try {
			...
			const visible = actor ? alerts.filter((a) => isDeliveryVisible(actor, a.delivery)) : alerts;
	...
	server.get(
		'/alerts/:id',
		{ preHandler: [requireScope('alerts:read')] },
		async (request, reply) => {
	...
	server.post(
		'/alerts/:id/evaluate',
		{ preHandler: [requireScope('alerts:run')] },
		async (request, reply) => {
			const { id } = request.params as { id: string };
			...
			if (actor && !canRunAlerts(actor)) {
				return reply
					.status(403)
					.send({ ok: false, error: 'Insufficient privileges to evaluate alerts.' });
			}
	...
	server.post(
		...
		async (request, reply) => {
			...
			if (fireActor && !canRunAlerts(fireActor)) {
				return reply
					.status(403)
					.send({ ok: false, error: 'Insufficient privileges to fire alerts.' });
			}
