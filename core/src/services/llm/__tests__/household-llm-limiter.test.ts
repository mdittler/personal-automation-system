...
describe('HouseholdLLMLimiter', () => {
    ...
    describe('checkCost()', () => {
        ...
        it.each([NaN, Infinity, -Infinity, -0.01])('rejects invalid estimatedCost = %s', (v) => {
    ...
    describe('reserveEstimated() — side-effect only, does NOT enforce cap', () => {
        ...
        it.each([NaN, Infinity, -0.01])('rejects invalid est = %s', (v) => {
    ...
    describe('releaseReservation()', () => {
        ...
        it('no-op for PLATFORM_NOOP_RESERVATION even with non-null actual', () => {
            limiter.releaseReservation(PLATFORM_NOOP_RESERVATION, 0.10);
    ...
    describe('constructor validation', () => {
        ...
        it.each([NaN, Infinity, -1])('rejects defaultHouseholdMonthlyCostCap = %s', (v) => {
        ...
        it.each([0, -1, 1.5, NaN, Infinity])(
            ...
            (v) => {
                expect(() => build({ defaultHouseholdRateLimit: { maxRequests: v, windowSeconds: 3600 } })).toThrow();
            },
        );
        ...
        it.each([0, -1, NaN, Infinity])(
            ...
            (v) => {
                expect(() => build({ defaultHouseholdRateLimit: { maxRequests: 200, windowSeconds: v } })).toThrow();
            },
        );
