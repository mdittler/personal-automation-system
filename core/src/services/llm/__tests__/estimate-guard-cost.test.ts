...
import { estimateGuardCost, approximateTokens, type PriceLookup } from '../estimate-guard-cost.js';
...
describe('estimateGuardCost', () => {
    it('classify upper bound is small (< 0.001 for fast tier, 500-char prompt)', () => {
        const cost = estimateGuardCost({ method: 'classify', tier: 'fast', prompt: 'x'.repeat(500) }, prices);
        expect(cost).toBeLessThan(0.001);
        expect(cost).toBeGreaterThan(0);
    });

    it('complete uses provided maxOutputTokens as upper bound', () => {
        const lo = estimateGuardCost({ method: 'complete', tier: 'fast', prompt: 'x'.repeat(400), maxOutputTokens: 100 }, prices);
        const hi = estimateGuardCost({ method: 'complete', tier: 'fast', prompt: 'x'.repeat(400), maxOutputTokens: 1000 }, prices);
    ...
    it('maxOutputTokens = 0 returns cost from input tokens only', () => {
        const cost = estimateGuardCost({ method: 'complete', tier: 'fast', prompt: 'x'.repeat(400), maxOutputTokens: 0 }, prices);
        // 100 input tokens * 0.001/1k = 0.0001
        expect(cost).toBeCloseTo(0.0001, 5);
    });

    it.each([NaN, -1, Infinity, 1.5])('rejects invalid maxOutputTokens = %s', (v) => {
        expect(() =>
            estimateGuardCost({ method: 'complete', tier: 'fast', prompt: 'hi', maxOutputTokens: v as number }, prices),
        ).toThrow();
    });
    ...
    describe('price fallback', () => {
        it('unknown tier → defaultReservationUsd with warn', () => {
            ...
            const warn = vi.fn();
            const cost = estimateGuardCost({ method: 'complete', tier: 'fast', prompt: 'hi' }, empty, { warn } as any);
            expect(cost).toBe(0.05);
            expect(warn).toHaveBeenCalled();
        });

        it('price with NaN field → fallback with warn', () => {
            const bad: PriceLookup = { priceFor: () => ({ inputUsdPer1k: NaN, outputUsdPer1k: 0.005 }) };
            const warn = vi.fn();
            const cost = estimateGuardCost({ method: 'complete', tier: 'fast', prompt: 'hi' }, bad, { warn } as any);
            expect(cost).toBe(0.05);
            expect(warn).toHaveBeenCalled();
        });

        it('price with negative field → fallback', () => {
            const bad: PriceLookup = { priceFor: () => ({ inputUsdPer1k: -0.001, outputUsdPer1k: 0.005 }) };
