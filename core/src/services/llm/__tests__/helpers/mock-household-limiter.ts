...
export function createMockHouseholdLimiter(overrides: Partial<MockHouseholdLimiter> = {}): MockHouseholdLimiter {
    return {
        ...
        check: vi.fn((hhId: string | undefined) => ({
            allowed: true,
            commit: vi.fn(),
            limit: !hhId || hhId === PLATFORM_SYSTEM_HOUSEHOLD_ID
                ? { maxRequests: Infinity, windowSeconds: Infinity }
