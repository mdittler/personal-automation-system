...
import type { SessionRow, MessageRow, InternalSearchFilters } from '../types.js';
...
function makeSessionRow(overrides: Partial<SessionRow> & { id: string; user_id: string }): SessionRow {
  return {
    household_id: null,
    source: 'telegram',
    started_at: '2026-01-01T00:00:00Z',
...
const LEGACY_SESSION: TestSession = {
  ...
  turns: [
    ...
    makeTurn('assistant', 'Pasta carbonara is a classic Roman dish. The guanciale adds rich flavor.', '2026-01-10T10:01:05Z'),
    makeTurn('user', 'What about the cheese?', '2026-01-10T10:02:00Z'),
    makeTurn('assistant', 'Pecorino Romano is traditional, though Parmesan is often substituted.', '2026-01-10T10:02:05Z'),
  ],
};
...
const HOUSEHOLD_SESSION: TestSession = {
  ...
  turns: [
    ...
    makeTurn('assistant', 'I can add olive oil and tomatoes to your grocery list.', '2026-01-11T09:01:05Z'),
...
const LEGACY_SESSION_2: TestSession = {
  ...
  turns: [
    ...
    makeTurn('assistant', 'I do not have real-time weather data available.', '2026-01-12T08:01:05Z'),
  ],
};
...
async function seedTranscripts(dataDir: string): Promise<void> {
  ...
  await writeTranscriptFile(
    join(dataDir, 'users', 'user-alice', 'chatbot', 'conversation', 'sessions', 'legacy-session-001.md'),
    buildTranscript(LEGACY_SESSION.meta, LEGACY_SESSION.turns),
  );

  // Legacy session 2
  await writeTranscriptFile(
    join(dataDir, 'users', 'user-alice', 'chatbot', 'conversation', 'sessions', 'legacy-session-002.md'),
    buildTranscript(LEGACY_SESSION_2.meta, LEGACY_SESSION_2.turns),
  );

  // Household session
  await writeTranscriptFile(
    join(dataDir, 'households', 'household-123', 'users', 'user-bob', 'chatbot', 'conversation', 'sessions', 'household-session-001.md'),
...
async function seedIndex(index: ChatTranscriptIndexImpl): Promise<void> {
  // Legacy session 1
  await index.upsertSession(makeSessionRow({
    id: 'legacy-session-001',
    user_id: 'user-alice',
    title: 'Alice discusses pasta',
    started_at: '2026-01-10T10:00:00Z',
  ...
  for (let i = 0; i < legacyTurns1.length; i++) {
    await index.appendMessage(makeMessageRow({
...
describe('rebuildIndex parity', () => {
  test('rebuilt index returns the same search results as a manually-seeded index', async () => {
    ...
    const queries: Array<{ filters: InternalSearchFilters; label: string }> = [
      {
        label: 'pasta (alice)',
        filters: { userId: 'user-alice', householdId: null, queryTerms: ['pasta'], limitSessions: 10 },
      },
      {
        label: 'carbonara (alice)',
        filters: { userId: 'user-alice', householdId: null, queryTerms: ['carbonara'], limitSessions: 10 },
      },
      {
        label: 'grocery olive oil (bob)',
        filters: { userId: 'user-bob', householdId: 'household-123', queryTerms: ['olive', 'oil'], limitSessions: 10 },
      },
      {
        label: 'weather (alice)',
        filters: { userId: 'user-alice', householdId: null, queryTerms: ['weather'], limitSessions: 10 },
      },
      {
        label: 'cross-user isolation: alice query returns nothing for bob',
        filters: { userId: 'user-bob', householdId: 'household-123', queryTerms: ['pasta'], limitSessions: 10 },
      },
    ];
...
describe('rebuildIndex corrupt file handling', () => {
  test('corrupt .md file is skipped without throwing', async () => {
    ...
    await writeTranscriptFile(
      join(dataDir, 'users', 'user-alice', 'chatbot', 'conversation', 'sessions', 'valid-session.md'),
      buildTranscript(
        makeSessionFrontmatter({ id: 'valid-session', user_id: 'user-alice', started_at: '2026-01-15T00:00:00Z' }),
    ...
    await writeTranscriptFile(
      join(dataDir, 'users', 'user-alice', 'chatbot', 'conversation', 'sessions', 'corrupt-session.md'),
      'this is not valid markdown frontmatter\nno yaml header present',
    );

    // Write another corrupt file (valid YAML front but no closing ---)
    await writeTranscriptFile(
      join(dataDir, 'users', 'user-alice', 'chatbot', 'conversation', 'sessions', 'truncated-session.md'),
      '---\nid: truncated\n',  // no closing ---
    );
...
describe('rebuildIndex idempotency', () => {
  test('running rebuild twice does not duplicate sessions or turns in search results', async () => {
    ...
    await rebuildIndex({ dbPath: rebuiltDb, dataDir });
    // Second run over same DB — upsertSession uses INSERT OR REPLACE and
    // appendMessage uses INSERT OR IGNORE, so results should be identical
