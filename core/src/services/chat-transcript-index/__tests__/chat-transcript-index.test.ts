...
describe('upsertSession + appendMessage + searchSessions', () => {
  ...
  test('insert session + 3 turns → searchSessions returns matching turn with snippet', async () => {
    ...
    await index.appendMessage(makeMessage({ session_id: 's1', turn_index: 0, role: 'user', content: 'I love pasta carbonara', timestamp: '2026-01-01T00:00:01Z' }));
    await index.appendMessage(makeMessage({ session_id: 's1', turn_index: 1, role: 'assistant', content: 'Pasta carbonara is a classic Italian dish', timestamp: '2026-01-01T00:00:02Z' }));
    await index.appendMessage(makeMessage({ session_id: 's1', turn_index: 2, role: 'user', content: 'How do I make pasta?', timestamp: '2026-01-01T00:00:03Z' }));
  ...
  test('auth filter: user A cannot see user B turns', async () => {
    ...
    await index.appendMessage(makeMessage({ session_id: 'sA', turn_index: 0, content: 'pasta recipe' }));
    await index.appendMessage(makeMessage({ session_id: 'sB', turn_index: 0, content: 'pasta sauce' }));
  ...
  test('userId empty string → no results (auth parameter required)', async () => {
    await index.upsertSession(makeSession({ id: 's1', user_id: 'u1' }));
    await index.appendMessage(makeMessage({ session_id: 's1', turn_index: 0, content: 'pasta carbonara' }));
  ...
  test('excludeSessionIds filters at SQL level', async () => {
    await index.upsertSession(makeSession({ id: 's1', user_id: 'u1' }));
    await index.upsertSession(makeSession({ id: 's2', user_id: 'u1', started_at: '2026-01-02T00:00:00Z' }));
    await index.appendMessage(makeMessage({ session_id: 's1', turn_index: 0, content: 'pasta recipe' }));
    await index.appendMessage(makeMessage({ session_id: 's2', turn_index: 0, content: 'pasta sauce' }));
  ...
  test('limitSessions caps results', async () => {
    ...
    for (let i = 0; i < 4; i++) {
      await index.upsertSession(makeSession({
        id: `s${i}`,
        user_id: 'u1',
        started_at: `2026-01-0${i + 1}T00:00:00Z`,
      }));
      await index.appendMessage(makeMessage({
        session_id: `s${i}`,
        turn_index: 0,
...
describe('deleteSession', () => {
  ...
  test('cascade: FTS rows gone after session deleted (search returns nothing)', async () => {
    await index.upsertSession(makeSession({ id: 's1', user_id: 'u1' }));
    await index.appendMessage(makeMessage({ session_id: 's1', turn_index: 0, content: 'pasta carbonara recipe' }));
...
describe('ordering', () => {
  ...
  test('sessions ordered by min(bm25) ASC, then sessionStartedAt DESC, then sessionId ASC', async () => {
    ...
    for (const s of sessions) {
      ...
      await index.appendMessage(makeMessage({ session_id: s.id, turn_index: 0, content: 'pasta cooking tips' }));
    }
  ...
  test('sessionId ASC is tiebreak when started_at is also equal', async () => {
    ...
    await index.appendMessage(makeMessage({ session_id: 'sZ', turn_index: 0, content: 'pasta delicious' }));
    await index.appendMessage(makeMessage({ session_id: 'sA', turn_index: 0, content: 'pasta delicious' }));
    await index.appendMessage(makeMessage({ session_id: 'sM', turn_index: 0, content: 'pasta delicious' }));
...
describe('WAL checkpoint', () => {
  test('after 50 writes, no errors thrown (file-backed DB)', async () => {
    ...
    try {
      ...
      for (let i = 0; i < 25; i++) {
        await idx.upsertSession(makeSession({ id: `s${i}`, user_id: 'u1', started_at: `2026-01-01T00:${String(i).padStart(2, '0')}:00Z` }));
        await idx.appendMessage(makeMessage({ session_id: `s${i}`, turn_index: 0, content: `message ${i}` }));
      }
  ...
  test('concurrent Promise.all appendMessage calls both succeed', async () => {
    ...
    try {
      ...
      const [r1, r2] = await Promise.all([
        idx.appendMessage(makeMessage({ session_id: 's1', turn_index: 1, role: 'user', content: 'msg1', timestamp: '2026-01-01T00:00:01Z' })),
        idx.appendMessage(makeMessage({ session_id: 's1', turn_index: 2, role: 'assistant', content: 'msg2', timestamp: '2026-01-01T00:00:02Z' })),
      ]);
...
describe('listExpiredSessions', () => {
  ...
  test('returns only sessions with ended_at < cutoff', async () => {
    await index.upsertSession(makeSession({ id: 's1', user_id: 'u1', started_at: '2026-01-01T00:00:00Z', ended_at: '2026-01-01T01:00:00Z' }));
    await index.upsertSession(makeSession({ id: 's2', user_id: 'u1', started_at: '2026-01-02T00:00:00Z', ended_at: '2026-01-05T01:00:00Z' }));
    await index.upsertSession(makeSession({ id: 's3', user_id: 'u1', started_at: '2026-01-03T00:00:00Z' })); // no ended_at
...
describe('upsertSession idempotency', () => {
  ...
  test('appendMessage with same turn_index is idempotent (INSERT OR IGNORE)', async () => {
    await index.upsertSession(makeSession({ id: 's1', user_id: 'u1' }));
    await index.appendMessage(makeMessage({ session_id: 's1', turn_index: 0, content: 'first content' }));
    await index.appendMessage(makeMessage({ session_id: 's1', turn_index: 0, content: 'second content — should be ignored' }));
...
describe('date range filters', () => {
  ...
  test('startedAfter filters sessions before the date', async () => {
    await index.upsertSession(makeSession({ id: 'old', user_id: 'u1', started_at: '2025-06-01T00:00:00Z' }));
    await index.upsertSession(makeSession({ id: 'new', user_id: 'u1', started_at: '2026-01-01T00:00:00Z' }));
    await index.appendMessage(makeMessage({ session_id: 'old', turn_index: 0, content: 'pasta recipe' }));
    await index.appendMessage(makeMessage({ session_id: 'new', turn_index: 0, content: 'pasta recipe' }));
