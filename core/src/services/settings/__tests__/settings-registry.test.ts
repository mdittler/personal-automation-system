...
describe('register validation (REQ-SETTINGS-006/008)', () => {
	...
	it('throws when nlSafe=true and nlIntentRegex is absent', () => {
		const reg = new SettingsRegistry();
		expect(() => reg.register(makeDef({ nlSafe: true }))).toThrow(
			/nlIntentRegex.*required.*nlSafe/i,
		);
	});

	it('throws when type=select and options is empty', () => {
		const reg = new SettingsRegistry();
		expect(() => reg.register(makeDef({ type: 'select', options: [] }))).toThrow(
			/options.*required.*select/i,
		);
	});
	...
	it('throws when dangerous=true and dangerConfirmPrompt is absent', () => {
		const reg = new SettingsRegistry();
		expect(() => reg.register(makeDef({ dangerous: true, adminOnly: true }))).toThrow(
			/dangerConfirmPrompt.*required.*dangerous/i,
		);
	});
	...
	it('rejects duplicate qualified key', () => {
		...
		expect(() => reg.register(makeDef({ appId: 'chatbot', key: 'log_to_notes' }))).toThrow(
			/duplicate.*chatbot\.log_to_notes/i,
		);
	});
	...
	it('allows the same key in two different apps', () => {
		const reg = new SettingsRegistry();
		reg.register(makeDef({ appId: 'food', key: 'default_store' }));
		expect(() => reg.register(makeDef({ appId: 'shopping', key: 'default_store' }))).not.toThrow();
	});
	...
	it('accepts nlSafe=false without nlIntentRegex', () => {
		const reg = new SettingsRegistry();
		expect(() => reg.register(makeDef({ nlSafe: false, nlIntentRegex: undefined }))).not.toThrow();
	});
});
