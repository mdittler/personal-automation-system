...
- Chicken $5.99`;
		...
		expect(result.relationships).toEqual([{ target: 'prices/walmart.md', type: 'related' }]);
	});
...
Content`;
		...
		const result = parseFileContent(content);
		expect(result.relationships).toEqual([{ target: 'recipes/original.yaml', type: 'source' }]);
	});
...
describe('resolveHouseholdMeta', () => {
	...
	it('household space path → householdId set, spaceKind household, collaborationId null', () => {
		const meta = resolveHouseholdMeta(
			'households/hh-alpha/spaces/family-space/food/recipes/pasta.yaml',
		);
		expect(meta).toEqual({
