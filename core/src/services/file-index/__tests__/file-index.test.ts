...
const makeScopes = (
	userPaths: string[],
	sharedPaths: string[],
): { user: ManifestDataScope[]; shared: ManifestDataScope[] } => ({
...
describe('FileIndexService', () => {
	...
	describe('rebuild', () => {
		...
		it('excludes archived files', async () => {
			...
			await writeDataFile(
				dataDir,
				'users/matt/food/recipes/tacos.2026-04-13_14-30-22.yaml',
				RECIPE_CONTENT,
			);
			await service.rebuild();
	...
	describe('getEntries filter', () => {
		...
		describe('date range filtering', () => {
			...
			it('dateFrom includes file when dateFrom is before latest date', () => {
				...
				expect(
					service
						.getEntries({ appId: 'food', dateFrom: '2026-03-01' })
						.some((e) => e.type === 'nutrition-log'),
				).toBe(true);
			});
			...
			it('dateFrom excludes file when dateFrom is after latest date', () => {
				...
				expect(
					service
						.getEntries({ appId: 'food', dateFrom: '2026-04-01' })
						.some((e) => e.type === 'nutrition-log'),
				).toBe(false);
			});
			...
			it('dateTo includes file when dateTo is after earliest date', () => {
				...
				expect(
					service
						.getEntries({ appId: 'food', dateTo: '2026-04-01' })
						.some((e) => e.type === 'nutrition-log'),
				).toBe(true);
			});
			...
			it('dateTo excludes file when dateTo is before earliest date', () => {
				...
				expect(
					service
						.getEntries({ appId: 'food', dateTo: '2026-03-01' })
						.some((e) => e.type === 'nutrition-log'),
				).toBe(false);
			});
		});
...
Content with [[recipes/tacos]].`;
	...
	describe('concurrency', () => {
		it('concurrent handleDataChanged calls on same file resolve without corruption', async () => {
			...
			await Promise.all([service.handleDataChanged(payload), service.handleDataChanged(payload)]);
...
Need to restock [[pantry]] with tortillas.`,
		...
		it('user scope food files have correct owner', () => {
			const userFiles = householdService.getEntries({
				scope: 'user',
				owner: 'matt',
				appId: 'food',
			});
