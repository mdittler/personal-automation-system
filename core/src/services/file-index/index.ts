...
import {
	...
	parseFileContent,
	parsePathMeta,
	resolveHouseholdMeta,
} from './entry-parser.js';
import type { FileIndexEntry, FileIndexFilter } from './types.js';

export type { FileIndexEntry, FileIndexFilter } from './types.js';
...
export class FileIndexService {
	...
	constructor(
		...
		private readonly appScopes: Map<
			string,
	...
	getEntries(filter: FileIndexFilter = {}): FileIndexEntry[] {
		...
		for (const entry of this.entries.values()) {
			...
			if (filter.dateFrom && (!entry.dates.latest || entry.dates.latest < filter.dateFrom))
				continue;
			if (filter.dateTo && (!entry.dates.earliest || entry.dates.earliest > filter.dateTo))
				continue;
