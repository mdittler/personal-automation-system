...
import { extractWikiLinks, parseFrontmatter } from '../../utils/frontmatter.js';
...
function toDateString(val: unknown): string | null {
	if (typeof val === 'string' && /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])/.test(val))
...
export function parseFileContent(content: string): ParsedContent {
	...
	// Dates
	const dateVals = [toDateString(meta.date), toDateString(meta.created)].filter(
		Boolean,
	) as string[];
	const dates = {
	...
	if (
		...
		(meta.source.includes('/') || /\.\w+$/.test(meta.source))
	) {
