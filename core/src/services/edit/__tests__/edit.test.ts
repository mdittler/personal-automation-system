...
function makeDataQueryService(
	files: Array<{ path: string; appId: string; content: string }>,
...
describe('EditServiceImpl', () => {
		...
		const interactionContext = makeInteractionContext([
			{
				appId: 'notes',
				action: 'open-file',
				filePaths: ['users/matt/notes/recent.md', 'users/matt/notes/shared.md'],
				scope: 'user',
				timestamp: Date.now(),
			},
			{
				appId: 'notes',
				action: 'open-file',
				filePaths: ['users/matt/notes/shared.md', 'users/matt/notes/older.md'],
				scope: 'user',
	...
	it('confirmEdit: matching hash → ok: true, file written, data:changed emitted, logs appended', async () => {
		...
		// audit log appended
		expect(editLog.append).toHaveBeenCalledWith(
			expect.objectContaining({
				outcome: 'confirmed',
				userId: 'matt',
				filePath: relativePath,
	...
	it('confirmEdit: access revoked between propose and confirm → ok: false, access_denied in log', async () => {
		...
		const revokedRegistry = makeAppRegistry({
			appId: 'food',
			scopePath: 'recipes/',
			access: 'read',
		});
	...
	it('confirmEdit: concurrent confirms — lock serializes, second sees stale hash', async () => {
		...
		const [r1, r2] = await Promise.all([svc.confirmEdit(proposal1), svc.confirmEdit(proposal2)]);
