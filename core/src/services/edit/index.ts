...
import { extractHouseholdIdFromPath, findMatchingScope } from '../data-store/paths.js';
...
export class EditServiceImpl implements EditService {
	...
	async proposeEdit(description: string, userId: string): Promise<ProposeEditResult> {
		...
		if (
			fileScopeSegments[0] === 'users' &&
			fileScopeSegments[1] !== 'shared' &&
			fileScopeSegments[1] !== userId
		) {
			return {
				kind: 'error',
				action: 'access_denied',
		...
		if (contextHouseholdId) {
			const pathHouseholdId = extractHouseholdIdFromPath(absolutePath);
		...
		const escapedContent = beforeContent.replace(/[\u0060\uFF40]{3,}/g, '\\`\\`\\`');
		...
		try {
			afterContent = await this.llm.complete(`${systemPrompt}\n\n${userPrompt}`, {
				tier: 'standard',
			});
		} catch (err) {
	...
	async confirmEdit(proposal: EditProposal): Promise<{ ok: true } | { ok: false; reason: string }> {
		// Step 1: Expiry check
		...
		if (confirmContextHouseholdId) {
			const pathHouseholdId = extractHouseholdIdFromPath(realAbsPath);
			if (pathHouseholdId !== null && pathHouseholdId !== confirmContextHouseholdId) {
				throw new HouseholdBoundaryError(
		...
		return withFileLock(realAbsPath, async () => {
			...
			try {
				...
				this.logger.error(
					`EditService confirmEdit: could not re-read "${proposal.absolutePath}": ${String(err)}`,
				);
