 ...
 *
 * Transparently propagates per-request metadata (currently `userId`) through
 * the async call stack so infrastructure consumers — LLM cost attribution,
 * `AppConfigService` per-user override lookups, and any future per-request
 * infrastructure (audit logging, feature flags, locale, …) — can read the
 * current request's user without apps having to pass it explicitly.
 *
 ...
 * per-user` jobs) via `requestContext.run({ userId }, fn)`.
 */
