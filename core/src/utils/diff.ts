...
function buildHunks(
	...
	while (i < ops.length) {
		...
		const contextAfter = Math.min(ops.length - 1, changeEnd + 2);
		...
		// Count lines in hunk
		let beforeStart = Number.POSITIVE_INFINITY;
		let beforeEnd = Number.NEGATIVE_INFINITY;
		let afterStart = Number.POSITIVE_INFINITY;
		let afterEnd = Number.NEGATIVE_INFINITY;
		...
		const hunk: Hunk = {
			beforeStart: beforeStart === Number.POSITIVE_INFINITY ? 0 : beforeStart + 1,
			beforeCount: beforeEnd === Number.NEGATIVE_INFINITY ? 0 : beforeEnd - beforeStart + 1,
			afterStart: afterStart === Number.POSITIVE_INFINITY ? 0 : afterStart + 1,
			afterCount: afterEnd === Number.NEGATIVE_INFINITY ? 0 : afterEnd - afterStart + 1,
