...
| `core/src/utils/model-id.ts` | `isValidModelId` — the single model-id validator (GUI tiers, `<switch-model>`, regression `--model-matrix`) |
...
| `docs/agentic-autonomy-doctrine.md` | Adopted doctrine (2026-07-07) binding all agentic work: per-session tier-gated autonomy, code-owned loop, no resident agent (AG-8) |
...
- **Truncation Diagnosis + Local-Model Cost Correctness** (2026-09-02) — closed the truncation-reported-as-malformed defect class with one shared `classifyStructuredOutput` helper (10 sites, ordering an explicit caller choice), extended the empty-output guard to the OpenAI-compatible/llama.cpp path, made local models estimate $0 in all three estimators, and aligned the model-id validators so namespaced ids assign to tiers. 6 new URS entries (REQ-LLM-041..043, REQ-REG-021/022, REQ-SEC-013).
- **Regression Run Unblock — Model Capability Guards + Live Progress** (2026-09-01) — unblocked a 100%-`error` regression sweep: per-model `temperature` capability gate + self-heal, Ollama thinking off by default, truncation reported as truncation at three call sites, receipt date-oracle unrotted, GUI live-run progress bar. 8 new URS entries (REQ-LLM-037..040, REQ-REG-019/020, REQ-REG-GUI-V2-027/028).
