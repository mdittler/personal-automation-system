...
All infrastructure (phases 0–30), food app (H1–H12b), security remediation (R1–R7, CR6, CR8, CR9), and deployment readiness (D1–D6 incl. D5a, D5b) phases are complete. LLM enhancement items #1 (route metadata) and #2 Chunks A–D complete. D5c Chunks 0–E complete. Hermes P1 Chunks A–D.3 complete. ~8000+ tests / ~310 files.
...
### Current Priority: Hermes P1 Chunk D.4 (next)
**Hermes P1 Chunks A–D.3** are complete. D.3 deleted `apps/chatbot/`, removed `chatbotApp`/`fallbackMode` from `RouterOptions`, removed `PROTECTED_APPS` guard on 'chatbot', rewrote manifest-parity test as virtual-manifest contract test, and dropped `fallback`/`_legacyKeys` from `SystemConfig`. Next: Hermes P1 Chunk D.4 (SystemInfoService cleanup — drop `fallbackMode` from `/gui/llm` template + `system-info/index.ts`). **LLM Enhancement #2 Chunks A–D** are complete: Chunk A (route-first dispatch, 9-intent allowlist, ~74 new tests), Chunk B (shadow classifier infrastructure B.1 + B.2 + FOOD_PERSONAS, ~239 tests), Chunk C (shadow pipeline wired into `handleMessage` — `computeVerdict`, `finalizeShadow`, `regexWinner` per branch, skipped-* gates, `shadow_sample_rate` config; +26 tests), Chunk D (shadow-primary machinery — `routing_primary` flag, `SHADOW_HANDLERS` 25-entry table, `shadow_min_confidence` threshold, result reuse preventing double-classify, telemetry CLI `pnpm analyze-shadow-log`; +64 tests). Production flip (`routing_primary: shadow`) is a config decision gated on ≥95% telemetry from `pnpm analyze-shadow-log`. Future work: LLM Enhancement #3 (structured extractors) or A.1 allowlist expansion.
