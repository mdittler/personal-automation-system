...
export function analyzeLog(markdown: string): LogStats {
	...
	const entries = blocks.map(parseShadowLogEntry).filter((e): e is ParsedEntry => e !== null);

	const verdicts: Record<string, number> = {};
	const perLabel = new Map<string, { agree: number; total: number }>();
	const disagreementPairs = new Map<
		string,
		{ regexLabel: string; shadowLabel: string; count: number }
	>();
	let judgmentTotal = 0;
	let agreeCount = 0;
...
async function main(): Promise<void> {
	...
	console.log(
		`Judgment entries (agree + disagree + one-side-none + both-none): ${stats.judgmentTotal}`,
	);
	...
	if (stats.perLabelAgreement.length > 0) {
		...
		for (const row of stats.perLabelAgreement.slice(0, 10)) {
			console.log(
				`  ${(row.rate * 100).toFixed(1).padStart(5)}%  (${row.agree}/${row.total})  ${row.label}`,
			);
		}
	}
