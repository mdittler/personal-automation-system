...
async function main(): Promise<void> {
  ...
  const retentionDaysRaw = parseInt(values['retention-days']!, 10);

  if (Number.isNaN(retentionDaysRaw) || retentionDaysRaw < 1 || retentionDaysRaw > 3650) {
    console.error(`Error: --retention-days must be an integer between 1 and 3650 (got: ${values['retention-days']})`);
    process.exit(1);
  }
