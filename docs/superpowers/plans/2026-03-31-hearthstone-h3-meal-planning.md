# Hearthstone H3: Meal Planning + "What Can I Make?" Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add weekly meal plan generation, "what's for dinner?" resolver, "what can I make?" pantry matcher, and the first Hearthstone scheduled cron job.
...
**Spec:** `docs/superpowers/specs/2026-03-31-hearthstone-h3-design.md`
...
| File | Responsibility |
|------|---------------|
| `apps/hearthstone/src/services/meal-plan-store.ts` | Meal plan CRUD, formatting, archive, tonight resolver |
| `apps/hearthstone/src/services/meal-planner.ts` | LLM plan generation, swap, new recipe detail generation |
| `apps/hearthstone/src/services/pantry-matcher.ts` | LLM "what can I make?" cross-reference |
| `apps/hearthstone/src/__tests__/meal-plan-store.test.ts` | Tests for plan CRUD and formatting |
| `apps/hearthstone/src/__tests__/meal-planner.test.ts` | Tests for LLM generation and swap |
| `apps/hearthstone/src/__tests__/pantry-matcher.test.ts` | Tests for matching logic |
...
| `apps/hearthstone/src/types.ts` | Add `isNew` and `description` to PlannedMeal |
| `apps/hearthstone/manifest.yaml` | Change `region` config to `location` |
| `apps/hearthstone/src/index.ts` | New commands, intents, callbacks, handleScheduledJob |
| `apps/hearthstone/help.md` | Add meal planning and pantry matcher docs |
| `apps/hearthstone/docs/urs.md` | Update requirement statuses |
| `apps/hearthstone/docs/implementation-phases.md` | Update H3 status |
...
**Files:**
- Modify: `apps/hearthstone/src/types.ts:80-89`
- Modify: `apps/hearthstone/manifest.yaml:244-247`

- [ ] **Step 1: Add isNew and description to PlannedMeal**

In `apps/hearthstone/src/types.ts`, update the `PlannedMeal` interface:
...
In `apps/hearthstone/manifest.yaml`, replace the `region` user_config entry:
...
```bash
git add apps/hearthstone/src/types.ts apps/hearthstone/manifest.yaml
git commit -m "feat(hearthstone): add PlannedMeal fields + location config for H3"
```
...
**Files:**
- Create: `apps/hearthstone/src/services/meal-plan-store.ts`
- Create: `apps/hearthstone/src/__tests__/meal-plan-store.test.ts`
...
describe('meal-plan-store', () => {
	...
	describe('buildPlanButtons', () => {
		it('returns grocery list and regenerate buttons', () => {
			...
			expect(buttons[0][0].callbackData).toBe('app:hearthstone:grocery-from-plan');
			expect(buttons[0][1].callbackData).toBe('app:hearthstone:regenerate-plan');
		});
	});
});
```
...
Run: `pnpm test --run apps/hearthstone/src/__tests__/meal-plan-store.test.ts`
...
export async function archivePlan(store: ScopedDataStore, plan: MealPlan): Promise<void> {
	...
	const frontmatter = generateFrontmatter({
		...
		date: isoNow(),
		tags: ['hearthstone', 'meal-plan', 'archive'],
	});
...
git commit -m "feat(hearthstone): add meal-plan-store service with CRUD and formatting"
```
...
**Files:**
- Create: `apps/hearthstone/src/services/meal-planner.ts`
- Create: `apps/hearthstone/src/__tests__/meal-planner.test.ts`
...
Run: `pnpm test --run apps/hearthstone/src/__tests__/meal-planner.test.ts`
Expected: FAIL — module not found.
...
```bash
git add apps/hearthstone/src/services/meal-planner.ts apps/hearthstone/src/__tests__/meal-planner.test.ts
git commit -m "feat(hearthstone): add meal-planner service with LLM generation, swap, and new recipe details"
```
...
**Files:**
- Create: `apps/hearthstone/src/services/pantry-matcher.ts`
- Create: `apps/hearthstone/src/__tests__/pantry-matcher.test.ts`
...
Run: `pnpm test --run apps/hearthstone/src/__tests__/pantry-matcher.test.ts`
Expected: FAIL — module not found.
...
```bash
git add apps/hearthstone/src/services/pantry-matcher.ts apps/hearthstone/src/__tests__/pantry-matcher.test.ts
git commit -m "feat(hearthstone): add pantry-matcher service for 'what can I make?' queries"
```
...
At the top of `apps/hearthstone/src/index.ts`, add after the existing imports:
...
async function handleWhatsForDinner(ctx: MessageContext): Promise<void> {
	...
	const buttons: import('@pas/core/types').InlineButton[][] = [
		[
			{ text: '📖 Full Recipe', callbackData: `app:hearthstone:show-recipe:${meal.date}` },
			{ text: '🔄 Swap', callbackData: `app:hearthstone:swap:${meal.date}` },
		],
	];
...
Add to `apps/hearthstone/src/__tests__/app.test.ts` — a new describe block for meal planning:
...
Run: `pnpm test --run apps/hearthstone/`
...
```bash
git add apps/hearthstone/src/index.ts apps/hearthstone/src/__tests__/app.test.ts
git commit -m "feat(hearthstone): integrate meal planning commands, intents, callbacks, and scheduled job"
```
...
Add new sections to `apps/hearthstone/help.md` for meal planning and "what can I make?":
...
```bash
git add apps/hearthstone/help.md apps/hearthstone/docs/urs.md apps/hearthstone/docs/implementation-phases.md
git commit -m "docs(hearthstone): update H3 documentation — help, URS, phases"
```
