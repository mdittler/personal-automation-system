# Food H5b Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add cooking timers, TTS/hands-free mode, and contextual food questions to Food cook mode.
...
**Spec:** `docs/superpowers/specs/2026-04-02-food-h5b-design.md`
...
**Files:**
- Create: `apps/food/src/services/timer-parser.ts`
- Create: `apps/food/src/__tests__/timer-parser.test.ts`
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/timer-parser.test.ts`
...
```bash
git add apps/food/src/services/timer-parser.ts apps/food/src/__tests__/timer-parser.test.ts
git commit -m "feat(food): add timer-parser service with parseStepTimer and formatDuration"
```
...
**Files:**
- Modify: `apps/food/src/types.ts` (lines 47-62)
- Modify: `apps/food/src/services/cook-session.ts` (lines 52-54, 109-118)
- Modify: `apps/food/src/__tests__/cook-session.test.ts`

- [ ] **Step 1: Write new cook-session tests**

Add the following tests to the end of `apps/food/src/__tests__/cook-session.test.ts` (before the closing, after the `getSessionCount` describe block at line 322):
...
describe('buildStepButtons with timer', () => {
	...
	it('returns 2 rows when timer is provided', () => {
		...
		expect(buttons[1][0].callbackData).toBe('app:food:ck:t');
	});
	...
	it('shows cancel button when session has active timer on current step', () => {
		...
		expect(buttons[1][0].callbackData).toBe('app:food:ck:tc');
		clearTimeout(session.timerHandle);
	});
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/cook-session.test.ts`
Expected: New tests fail (type errors and missing parameters).

- [ ] **Step 2: Add timer fields to CookSession type**

In `apps/food/src/types.ts`, add three optional fields to the `CookSession` interface (after `lastChatId` at line 61):
...
In `apps/food/src/services/cook-session.ts`, add the import for ParsedTimer and update `buildStepButtons`:
...
In `apps/food/src/services/cook-session.ts`, add the timer declaration at the top (after line 8, before the `EXPIRY_MS` constant) and update `endSession` (lines 52-54):
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/cook-mode-handler.test.ts`
...
```bash
git add apps/food/src/types.ts apps/food/src/services/cook-session.ts apps/food/src/__tests__/cook-session.test.ts
git commit -m "feat(food): extend CookSession with timer/TTS fields, update buildStepButtons for timer row"
```
...
- Create: `apps/food/src/__tests__/cook-timer.test.ts`
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/cook-timer.test.ts`
Expected: Tests fail (timer callback handlers not yet implemented).

- [ ] **Step 2: Add timer handling to cook-mode.ts**

In `apps/food/src/handlers/cook-mode.ts`, make the following changes:
...
```bash
git add apps/food/src/handlers/cook-mode.ts apps/food/src/__tests__/cook-timer.test.ts
git commit -m "feat(food): add cooking timer handlers with auto-cancel on navigation"
```
...
- Create: `apps/food/src/__tests__/cook-tts.test.ts`
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/cook-tts.test.ts`
Expected: Tests fail (hands-free callbacks not yet implemented).

- [ ] **Step 2: Add hands-free prompt to handleServingsReply**

In `apps/food/src/handlers/cook-mode.ts`, update `handleServingsReply`. After the ingredients message is sent and before the first step, add the hands-free prompt logic. Replace the end of `handleServingsReply` (from "Create session" onward):
...
In `apps/food/manifest.yaml`, add two new config fields at the end of the `user_config` section (after line 269):
...
```bash
git add apps/food/src/handlers/cook-mode.ts apps/food/src/__tests__/cook-tts.test.ts apps/food/manifest.yaml
git commit -m "feat(food): add TTS/hands-free mode with speaker config and prompt flow"
```
...
**Files:**
- Modify: `apps/food/src/index.ts` (lines 1018-1032)
- Create: `apps/food/src/__tests__/contextual-food-question.test.ts`
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/contextual-food-question.test.ts`
Expected: Tests fail (handleFoodQuestion not yet enhanced).

- [ ] **Step 2: Enhance handleFoodQuestion in index.ts**

In `apps/food/src/index.ts`, add the import for `getSession` from the cook-session service (add to existing cook-mode import or add a new import):
...
```bash
git add apps/food/src/index.ts apps/food/src/__tests__/contextual-food-question.test.ts
git commit -m "feat(food): enhance food questions with context store and active session context"
```
...
- [ ] **Step 1: Run full Food test suite**

Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/`
Expected: All tests pass (~53 existing + ~45 new = ~98 total in Food).
...
```bash
git add -A
git commit -m "chore(food): H5b final verification — timers, TTS, contextual food questions"
```
