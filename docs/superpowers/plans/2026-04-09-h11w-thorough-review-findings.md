...
**Status:** Fixed — `validate-label` helper added; slug validity checked via try/catch at the `awaiting_label` step and again defensively in the confirm-save callback.
...
**Status:** Fixed — `sanitizeForPrompt()` added to `sanitize.ts` with newline stripping and fence sentinel scrubbing; applied to all three user fields in `estimateMacros`.
...
**Status:** Fixed — `MAX_INGREDIENTS=50`, `MAX_INGREDIENT_LEN=200`, and `MAX_PROMPT_BODY_LEN=5000` constants enforced in `estimateMacros`; handler rejects excess with a user-facing message.
...
**Status:** Fixed — `signedDaysDiff` function replaces `Math.abs`; `isWithinWindow` uses `>= 0 && <= WINDOW_DAYS` check, dropping future-dated entries.
...
**Status:** Fixed — inline trim called in `recordAdHocLog` with 90-day window, plus 500-entry FIFO cap applied after trim.
...
**Status:** Fixed — `AsyncLock` applied around `saveQuickMeal`, `archiveQuickMeal`, and `incrementUsage` in `quick-meals-store.ts`; same lock pattern applied to `recordAdHocLog` in `ad-hoc-history.ts`.
...
**Status:** Fixed — `validateLabel` called in `beginQuickMealAddPrefilled` with an H5-fix comment; rejects invalid labels before writing pending state.
...
**Status:** Fixed — `escapeMarkdown` applied to all 4 remaining unescaped spots in `nutrition.ts` (meal log confirmation, quick-meal log confirmation, today's summary title, and macro display).
...
**Status:** Fixed — `<=` used in filter (via `slice(0, MAX_INGREDIENTS)` with `MAX_INGREDIENT_LEN=200`), and user receives feedback when lines are rejected.
...
**Status:** Fixed — shared `STOP_WORDS` from `utils/stopwords.ts` used in `ad-hoc-history.ts` tokenizer, removing common English function words from similarity scoring.
...
**Status:** Fixed — NFKD normalization with `\p{Diacritic}` removal and unicode-aware regex applied in `recipe-matcher.ts` tokenizer.
...
**Status:** Fixed — `ARCHIVE_MAX=500` cap with FIFO eviction applied in `archiveQuickMeal`.
...
**Status:** Fixed — time-based sweep with `expiresAt` TTL replaces the unsafe LRU eviction in both `pendingPromotion` and `quick-meal-flow.ts`.
...
**Status:** Fixed — `usda-fdc-client.ts` deleted; TODO markers removed from `quick-meal-flow.ts`.
...
**Status:** Fixed — strict decimal regex `^-?\d+(?:\.\d+)?$` applied before `Number()` in `parsePortion`.
...
**Status:** Closed as accepted risk — `sanitizeInput` is designed for display contexts; `sanitizeForPrompt` is used on fence-delimited paths in the estimator. JSDoc added to `sanitizeInput` clarifying the scope distinction.
...
**Status:** Fixed — German, French, and Spanish glue words added to `STOP_WORDS` in `utils/stopwords.ts`.
