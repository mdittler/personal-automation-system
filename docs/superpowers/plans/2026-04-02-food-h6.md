# Food H6: Leftovers, Freezer, Waste & Perishable Alerts — Implementation Plan
...
| File | Responsibility |
|------|---------------|
| `apps/food/src/services/leftover-store.ts` | CRUD, formatting, expiry queries for leftovers |
| `apps/food/src/services/freezer-store.ts` | CRUD, formatting, aging queries for freezer |
| `apps/food/src/services/waste-store.ts` | Append-only waste log |
| `apps/food/src/handlers/leftover-handler.ts` | Leftover callbacks + daily check job |
| `apps/food/src/handlers/freezer-handler.ts` | Freezer callbacks + Monday check job |
| `apps/food/src/handlers/perishable-handler.ts` | Perishable callbacks + daily pantry check |
| `apps/food/src/__tests__/services/leftover-store.test.ts` | Unit tests |
| `apps/food/src/__tests__/services/freezer-store.test.ts` | Unit tests |
| `apps/food/src/__tests__/services/waste-store.test.ts` | Unit tests |
| `apps/food/src/__tests__/handlers/leftover-handler.test.ts` | Handler tests |
| `apps/food/src/__tests__/handlers/freezer-handler.test.ts` | Handler tests |
| `apps/food/src/__tests__/handlers/perishable-handler.test.ts` | Handler tests |

### Modified Files
| File | Change |
|------|--------|
| `apps/food/src/types.ts` | Add `WasteLogEntry` interface |
| `apps/food/src/index.ts` | Add commands, callback routing, intent detection, scheduled job dispatch, pending state maps |
| `apps/food/src/handlers/rating.ts` | Add post-rating leftover prompt |
| `apps/food/src/handlers/cook-mode.ts` | Add post-cook leftover prompt |
| `apps/food/src/services/pantry-store.ts` | Add perishable expiry estimation |
...
**Files:**
- Modify: `apps/food/src/types.ts:182`
...
git commit -m "feat(food): add WasteLogEntry type for H6"
```
...
**Files:**
- Create: `apps/food/src/services/leftover-store.ts`
- Create: `apps/food/src/__tests__/services/leftover-store.test.ts`

- [ ] **Step 1: Create test directory**

```bash
mkdir -p apps/food/src/__tests__/services
```
...
describe('leftover-store', () => {
	...
	describe('buildLeftoverButtons', () => {
		it('creates action buttons for each active item', () => {
			...
			// Per-item rows
			expect(buttons[1]?.[0]?.callbackData).toBe('app:food:lo:use:0');
			expect(buttons[1]?.[1]?.callbackData).toBe('app:food:lo:freeze:0');
			expect(buttons[1]?.[2]?.callbackData).toBe('app:food:lo:toss:0');
		});
...
Run: `cd apps/food && npx vitest run src/__tests__/services/leftover-store.test.ts`
...
export function buildLeftoverButtons(
	...
	// Add button
	buttons.push([{ text: '➕ Add Leftovers', callbackData: 'app:food:lo:add' }]);
	...
	for (let i = 0; i < items.length; i++) {
		...
		buttons.push([
			{ text: `✅ ${name}`, callbackData: `app:food:lo:use:${i}` },
			{ text: `🧊 ${name}`, callbackData: `app:food:lo:freeze:${i}` },
			{ text: `🗑 ${name}`, callbackData: `app:food:lo:toss:${i}` },
		]);
	}
...
```bash
git add apps/food/src/services/leftover-store.ts apps/food/src/__tests__/services/leftover-store.test.ts
git commit -m "feat(food): add leftover-store service with tests"
```
...
**Files:**
- Create: `apps/food/src/services/freezer-store.ts`
- Create: `apps/food/src/__tests__/services/freezer-store.test.ts`
...
describe('freezer-store', () => {
	...
	describe('buildFreezerButtons', () => {
		it('creates thaw and toss buttons per item', () => {
			...
			expect(buttons[0]?.[0]?.text).toContain('Add');
			expect(buttons[1]?.[0]?.callbackData).toBe('app:food:fz:thaw:0');
			expect(buttons[1]?.[1]?.callbackData).toBe('app:food:fz:toss:0');
		});
	});
...
Run: `cd apps/food && npx vitest run src/__tests__/services/freezer-store.test.ts`
Expected: FAIL — module not found
...
export function buildFreezerButtons(
	...
	buttons.push([{ text: '➕ Add to Freezer', callbackData: 'app:food:fz:add' }]);

	for (let i = 0; i < items.length; i++) {
		const name = items[i]!.name.length > 10 ? items[i]!.name.slice(0, 10) + '…' : items[i]!.name;
		buttons.push([
			{ text: `🔥 ${name}`, callbackData: `app:food:fz:thaw:${i}` },
			{ text: `🗑 ${name}`, callbackData: `app:food:fz:toss:${i}` },
		]);
	}
...
```bash
git add apps/food/src/services/freezer-store.ts apps/food/src/__tests__/services/freezer-store.test.ts
git commit -m "feat(food): add freezer-store service with tests"
```
...
**Files:**
- Create: `apps/food/src/services/waste-store.ts`
- Create: `apps/food/src/__tests__/services/waste-store.test.ts`
...
Run: `cd apps/food && npx vitest run src/__tests__/services/waste-store.test.ts`
Expected: FAIL
...
```bash
git add apps/food/src/services/waste-store.ts apps/food/src/__tests__/services/waste-store.test.ts
git commit -m "feat(food): add waste-store service with tests"
```
...
**Files:**
- Create: `apps/food/src/handlers/leftover-handler.ts`
- Create: `apps/food/src/__tests__/handlers/leftover-handler.test.ts`
...
Run: `cd apps/food && npx vitest run src/__tests__/handlers/leftover-handler.test.ts`
Expected: FAIL
...
export async function handleLeftoverCheckJob(
	...
	for (const { item, idx } of expiringToday) {
		...
		buttons.push([
			{ text: `🧊 Freeze ${name}`, callbackData: `app:food:lo:freeze:${idx}` },
			{ text: `✅ Eat ${name}`, callbackData: `app:food:lo:keep:${idx}` },
			{ text: `🗑 Toss ${name}`, callbackData: `app:food:lo:toss:${idx}` },
		]);
	}
	for (const { item, idx } of expiringTomorrow) {
		...
		buttons.push([
			{ text: `🧊 Freeze ${name}`, callbackData: `app:food:lo:freeze:${idx}` },
			{ text: `✅ Got it`, callbackData: `app:food:lo:keep:${idx}` },
		]);
	}
...
```bash
git add apps/food/src/handlers/leftover-handler.ts apps/food/src/__tests__/handlers/leftover-handler.test.ts
git commit -m "feat(food): add leftover handler with callbacks and daily check job"
```
...
**Files:**
- Create: `apps/food/src/handlers/freezer-handler.ts`
- Create: `apps/food/src/__tests__/handlers/freezer-handler.test.ts`
...
Run: `cd apps/food && npx vitest run src/__tests__/handlers/freezer-handler.test.ts`
Expected: FAIL
...
```bash
git add apps/food/src/handlers/freezer-handler.ts apps/food/src/__tests__/handlers/freezer-handler.test.ts
git commit -m "feat(food): add freezer handler with callbacks and Monday check job"
```
...
**Files:**
- Create: `apps/food/src/handlers/perishable-handler.ts`
- Create: `apps/food/src/__tests__/handlers/perishable-handler.test.ts`
...
Run: `cd apps/food && npx vitest run src/__tests__/handlers/perishable-handler.test.ts`
Expected: FAIL
...
export async function handlePerishableCheckJob(
	...
	for (const { item, idx } of expiring) {
		...
		if (daysLeft <= 0) {
			buttons.push([
				{ text: `🧊 Freeze ${name}`, callbackData: `app:food:pa:freeze:${idx}` },
				{ text: `🗑 Toss`, callbackData: `app:food:pa:toss:${idx}` },
				{ text: `👍 Still good`, callbackData: `app:food:pa:ok:${idx}` },
			]);
		} else {
			buttons.push([
				{ text: `🧊 Move to Freezer`, callbackData: `app:food:pa:freeze:${idx}` },
				{ text: `👍 Still good`, callbackData: `app:food:pa:ok:${idx}` },
			]);
		}
	}
...
```bash
git add apps/food/src/handlers/perishable-handler.ts apps/food/src/__tests__/handlers/perishable-handler.test.ts
git commit -m "feat(food): add perishable handler with pantry expiry alerts"
```
...
git commit -m "feat(food): integrate H6 — commands, callbacks, intents, scheduled jobs"
```
...
**Files:**
- Modify: `apps/food/src/handlers/rating.ts:146`
- Modify: `apps/food/src/handlers/cook-mode.ts:383`
...
```typescript
	...
	if (direction !== 'skip' && meal.recipeTitle) {
		await services.telegram.sendWithButtons(
			...
			[[
				{ text: 'Yes, log leftovers', callbackData: 'app:food:lo:post-meal:yes' },
...
Run: `cd apps/food && npx vitest run src/__tests__/rating-handler.test.ts src/__tests__/cook-mode-handler.test.ts`
Expected: PASS (existing tests may need minor updates for the new `sendWithButtons` call)

- [ ] **Step 4: Commit**

```bash
git add apps/food/src/handlers/rating.ts apps/food/src/handlers/cook-mode.ts
git commit -m "feat(food): add post-rating and post-cook leftover prompts"
```
...
**Files:**
- Modify: `apps/food/src/services/pantry-store.ts`
- Modify: `apps/food/src/index.ts` (the `pantry-all` callback)
...
Add to `apps/food/src/__tests__/services/leftover-store.test.ts` (or create a new section in pantry-store.test.ts):

In `apps/food/src/__tests__/pantry-store.test.ts`, add:
...
Run: `cd apps/food && npx vitest run src/__tests__/pantry-store.test.ts`
Expected: All PASS

- [ ] **Step 5: Commit**

```bash
git add apps/food/src/services/pantry-store.ts apps/food/src/__tests__/pantry-store.test.ts apps/food/src/index.ts
git commit -m "feat(food): add perishable expiry estimation for pantry items"
```
...
**Files:**
- Modify: `apps/food/src/__tests__/natural-language.test.ts`
...
Run: `cd apps/food && npx vitest run src/__tests__/natural-language.test.ts`
Expected: All PASS

- [ ] **Step 3: Commit**

```bash
git add apps/food/src/__tests__/natural-language.test.ts
git commit -m "test(food): add H6 natural language intent tests"
```
...
- [ ] **Step 1: Run the full Food test suite**
...
git commit -m "docs: mark Food H6 complete — leftovers, freezer, waste, perishable alerts"
```
