# Food H4: Voting, Ratings, and Shopping Follow-up — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add meal plan voting, post-meal ratings with recipe confirmation, and shopping follow-up to the Food app.
...
**Spec:** `docs/superpowers/specs/2026-03-31-food-h4-design.md`
...
**Files:**
- Modify: `apps/food/src/types.ts:93-101`

- [ ] **Step 1: Add votingStartedAt and lastRatingPromptDate to MealPlan**

In `apps/food/src/types.ts`, add two optional fields to the `MealPlan` interface:
...
git commit -m "feat(food): add votingStartedAt and lastRatingPromptDate to MealPlan type"
```
...
**Files:**
- Create: `apps/food/src/services/voting.ts`
- Create: `apps/food/src/__tests__/voting.test.ts`
...
describe('voting service', () => {
	...
	describe('buildVoteButtons', () => {
		it('returns buttons with correct callback data', () => {
			...
			expect(buttons[0][0].callbackData).toBe('app:food:vote:up:2026-04-01');
			expect(buttons[0][1].callbackData).toBe('app:food:vote:down:2026-04-01');
			expect(buttons[0][2].callbackData).toBe('app:food:vote:neutral:2026-04-01');
		});
	});
});
```
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/voting.test.ts`
...
export function buildVoteButtons(mealDate: string): InlineButton[][] {
	return [
		[
			{ text: '👍', callbackData: `app:food:vote:up:${mealDate}` },
			{ text: '👎', callbackData: `app:food:vote:down:${mealDate}` },
			{ text: '😐', callbackData: `app:food:vote:neutral:${mealDate}` },
		],
	];
}
```
...
```bash
git add apps/food/src/services/voting.ts apps/food/src/__tests__/voting.test.ts
git commit -m "feat(food): add voting service with pure vote logic and tests"
```
...
**Files:**
- Create: `apps/food/src/handlers/voting.ts`
- Create: `apps/food/src/__tests__/voting-handler.test.ts`
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/voting-handler.test.ts`
...
```bash
git add apps/food/src/handlers/voting.ts apps/food/src/__tests__/voting-handler.test.ts
git commit -m "feat(food): add voting handler with send, callback, and finalize logic"
```
...
**Files:**
- Create: `apps/food/src/services/rating.ts`
- Create: `apps/food/src/__tests__/rating.test.ts`
...
describe('rating service', () => {
	...
	describe('buildRatingPromptButtons', () => {
		it('creates one button per uncooked meal', () => {
			...
			expect(buttons[1][0].callbackData).toBe('app:food:cooked:2026-04-01');
		});
	});
	...
	describe('buildRateButtons', () => {
		it('returns thumbs up, down, and skip buttons', () => {
			...
			expect(buttons[0][0].callbackData).toBe('app:food:rate:up:2026-03-31');
			expect(buttons[0][1].callbackData).toBe('app:food:rate:down:2026-03-31');
			expect(buttons[0][2].callbackData).toBe('app:food:rate:skip:2026-03-31');
		});
	});
});
```
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/rating.test.ts`
...
export function buildRateButtons(mealDate: string): InlineButton[][] {
	return [
		[
			{ text: '👍', callbackData: `app:food:rate:up:${mealDate}` },
			{ text: '👎', callbackData: `app:food:rate:down:${mealDate}` },
			{ text: '⏭ Skip', callbackData: `app:food:rate:skip:${mealDate}` },
		],
	];
}
```
...
```bash
git add apps/food/src/services/rating.ts apps/food/src/__tests__/rating.test.ts
git commit -m "feat(food): add rating service with uncooked meal queries and formatting"
```
...
**Files:**
- Create: `apps/food/src/handlers/rating.ts`
- Create: `apps/food/src/__tests__/rating-handler.test.ts`
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/rating-handler.test.ts`
...
```bash
git add apps/food/src/handlers/rating.ts apps/food/src/__tests__/rating-handler.test.ts
git commit -m "feat(food): add rating handler with cooked/rate callbacks and nightly prompt"
```
...
**Files:**
- Create: `apps/food/src/handlers/shopping-followup.ts`
- Create: `apps/food/src/__tests__/shopping-followup.test.ts`
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/shopping-followup.test.ts`
...
```bash
git add apps/food/src/handlers/shopping-followup.ts apps/food/src/__tests__/shopping-followup.test.ts
git commit -m "feat(food): add shopping follow-up handler with timer and clear/keep callbacks"
```
...
**Files:**
- Modify: `apps/food/src/services/meal-plan-store.ts:224-233`
- Modify: `apps/food/src/__tests__/app.test.ts` (add test)
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/meal-plan-store.test.ts` (or wherever this test was added)
Expected: FAIL — `buildPlanButtons` doesn't accept a `plan` parameter yet

- [ ] **Step 3: Update buildPlanButtons in meal-plan-store.ts**

In `apps/food/src/services/meal-plan-store.ts`, replace the `buildPlanButtons` function (lines 224-233):
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/src/__tests__/meal-plan-store.test.ts`
Expected: PASS
...
git commit -m "feat(food): add Cooked buttons to meal plan view via buildPlanButtons(plan)"
```
...
Run: `cd "C:/Users/matth/Projects/Personal Assistant" && pnpm vitest run apps/food/`
...
git commit -m "feat(food): wire H4 voting, rating, and shopping follow-up into app routing"
```
...
In `apps/food/manifest.yaml`, under `capabilities.schedules` (after the `generate-weekly-plan` entry around line 79), add:
...
git commit -m "feat(food): add finalize-votes and nightly-rating-prompt scheduled jobs to manifest"
```
...
git commit -m "test(food): add H4 integration tests for voting, rating, and shopping follow-up"
```
...
```bash
git add apps/food/docs/ apps/food/help.md
git commit -m "docs(food): update H4 implementation phases, help, and URS"
```
