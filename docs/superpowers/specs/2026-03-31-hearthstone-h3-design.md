# Hearthstone Phase H3: Meal Planning + "What Can I Make?"
...
New fields in Hearthstone user config (stored via app config system):
...
Hearthstone reads this from system config. Used in the LLM prompt for seasonal produce awareness. The LLM naturally knows what's in season for a given location and time of year — no static dataset needed.
...
|---|---|
| `app:hearthstone:grocery-from-plan` | Generate grocery list from all plan recipes |
| `app:hearthstone:regenerate-plan` | Regenerate entire plan |
| `app:hearthstone:show-recipe:<day>` | Show full recipe for a day (triggers detail generation for new suggestions) |
| `app:hearthstone:swap:<day>` | Swap a specific day's meal |
