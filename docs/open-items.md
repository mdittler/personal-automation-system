...
| 3c | ~~**Persona Regression Suite Chunk C — recall bucket + rubric oracle + chatbot bucket**~~ ✓ Complete (2026-05-11) | 1 | 25 recall cases (REQ-REG-005), rubric oracle (`oracles/rubric.ts`), 10 chatbot cases migrated from v0 corpus, chatbot env factory (REQ-REG-006/012), `--model-matrix` + `--judge-model` CLI flags. 329 tests in regression workspace. Carry-forwards: chatbot env `captureHandler`/`endActiveSession` placeholders need Router instrumentation; Task 14 operator-driven verification pending. |
...
- **Persona Regression Suite Chunk A.2 carry-forward — operator photo delivery + downstream tasks** — Substrate (Chunk A.1) shipped 2026-05-09. Chunk A.2 cannot start until 5 receipt photos are committed. The plan is in `~/.claude/plans/can-you-start-the-bright-frog.md` Tasks 11–15.
  ...
  **Deferred minors from the simplify pass** (`refactor(regression-A): post-Chunk-A simplify pass`, commit `63548fe`):
  - Stringly-typed verdict literals across regression tests — ✓ Partially resolved in Chunk B.1 (added `VERDICT` constants; production code migrated). Test files still use bare strings; refactor when scope expands.
...
- **Regression Suite Chunk C — `captureHandler` and `endActiveSession` placeholders** — Task 12 (`build-deps.ts:131-145`) ships `chatbotEnvFactory` with placeholder stubs: `captureHandler()` returns `null` and `endActiveSession()` is a no-op because `chatSessions`/`eventBus` aren't surfaced on the public `RuntimeServices` interface. Consequence: every chatbot case that declares `expectedHandler` (all 10 in Task 10) trips the chatbot-runner's "routing diagnostic captured no handler" branch and emits `verdict: 'error'` regardless of reply quality. Task 14's chatbot-bucket runs cannot produce meaningful findings until both are wired through real Router instrumentation. Resolution: instrument the Router to record the last-invoked handler id (or extend `RuntimeServices` to expose `chatSessions`/`eventBus`), then wire both stubs to real implementations. Trigger: operator runs Task 14 chatbot-bucket smoke and the placeholders surface as the dominant failure mode.
