...
- **Persona Regression Suite Chunk A.2 carry-forward — operator photo delivery + downstream tasks** — Substrate (Chunk A.1) shipped 2026-05-09. Chunk A.2 cannot start until 5 receipt photos are committed. The plan is in `~/.claude/plans/can-you-start-the-bright-frog.md` Tasks 11–15.
  ...
  **Deferred minors from the simplify pass** (`refactor(regression-A): post-Chunk-A simplify pass`, commit `63548fe`):
  ...
  - ~~`RunResult.tokenCounts` hardcoded to `{input: 0, output: 0}`~~ ✓ Resolved in Chunk B.1 — `runRoutingCase` aggregates `meter.tokenIn/tokenOut` per input. Receipt-runner still has it hardcoded; resolve when A.2 lands.
...
- **Persona Regression Suite Chunk B.1 carry-forward — real-LLM verification deferred to operator** — Chunk B.1 shipped 2026-05-11. The orchestrator + routing runner + 36 cases + CLI are all unit-tested and the CLI exercises the dry-run path end-to-end. **What's deferred:** wiring a real LLMService into `regression/src/runner/build-deps.ts`. Today, `composeMinimalLLMService()` returns a stub that throws on `complete()` — the operator must either (a) extend `build-deps.ts` to compose the full production LLMService stack (mirroring core/src/bootstrap.ts), or (b) invoke the CLI from a context that already has a wired LLM. Once wired, the operator should:
  - `pnpm test:regression -- --bucket=routing` and confirm ≥ 0.95 accuracy across food-shadow inputs (exit 0).
