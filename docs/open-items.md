...
- **Hermes P2: ContextStore double-read** — `gatherContext` (app-data.ts) calls `contextStore.listForUser` independently of the snapshot path. When both paths are active in production, the same ContextStore is read twice per request. Fix by passing snapshot context entries into `gatherContext`. Deferred post-P2.
