...
- **Hermes P5 carry-forward — Temporal precision** — P5 has coarse `startedAfter`/`startedBefore` ISO-date filters. Better temporal parsing (e.g. "last Tuesday", "two weeks ago") is a future LLM enhancement to the recall classifier.
...
- **Hermes P5 carry-forward — LLM-visible session_search tool** — Exposing transcript search as an explicit LLM tool call (instead of auto-injecting) is deferred to a future Hermes phase.
- **Hermes P5 carry-forward — `/recall` or `/search` slash command** — A user-facing `/recall <query>` or `/search <query>` command to search transcripts on demand is deferred to a future Hermes phase.
...
- ~~**Hermes P9 — Photo Memory Bridge + Receipt Integrity**~~ ✓ Complete (2026-04-29, 9dba662) — Photo handlers return `photoSummary`; router appends to session transcript; `formatConversationHistory` whitelist exempts photo-summary turns (2000-char, spoof-resistant); `PHOTO_SUMMARY_GUIDANCE` in chatbot prompt; `sanitizePhotoField` strips control chars + bidi + XML fence. Receipt: `isValidReceiptDate` (calendar-strict, 90-day window), `capturedAt` sort authority, `rawExtractedDate` audit trail, price-store `updatedAt` from `capturedAt`. REQ-CONV-PHOTO-001..005, REQ-FOOD-RECEIPT-001..002. LLM Enhancement #3.A/B/C complete.
