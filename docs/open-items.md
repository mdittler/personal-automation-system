...
- ~~**Hermes P5 — SQLite + FTS5 transcript search**~~ ✓ Complete (2026-04-28) — Full-text search across session transcripts via derived SQLite + FTS5 index. LLM recall classifier + auto-injection into Layer 5 fenced context (`<memory-context label="recalled-session">`). Rebuild CLI + opt-in retention. REQ-CONV-SEARCH-001..014. Spec: `docs/superpowers/specs/2026-04-28-hermes-p5-transcript-search-design.md`. Tests: `transcript-recall.persona.test.ts` (17 scenarios S1–S16), `transcript-recall.integration.test.ts` (6 scenarios T1–T3).
