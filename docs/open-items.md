...
- **Hermes P1 Chunk C** — Move `/ask`, `/edit` to built-in router commands; add `/notes` built-in; daily-note logging as per-user opt-in (`chat.log_to_notes`, default OFF); three-way toggle (slash command, GUI, `<config-set>` LLM tag with allowlist + intent-regex); `coerceUserConfigValue` shared between GUI POST and `<config-set>`; `/help` lists conversation commands once. Spec: `docs/superpowers/specs/2026-04-25-hermes-p1-conversation-service-design.md` (REQ-CONV-006 through REQ-CONV-010, REQ-CONV-016, REQ-CONV-019, REQ-CONV-020). Needs plan before coding; depends on Chunk B being merged.
...
- **Hermes P1 Chunk D: chatbot app removal** — Delete `apps/chatbot/`, remove `chatbotApp`/`fallbackMode` from `RouterOptions`, remove `PROTECTED_APPS` guard on 'chatbot' in `core/src/cli/uninstall-app.ts`, migrate the 80 chatbot app tests to `ConversationService` unit tests or delete them, then make `ConversationService.handleMessage` throw as a regression detector for any remaining shim-path. Also remove notes-mode routing path (`fallbackMode: 'notes'`) which is fully preserved in Chunks B–C for back-compat. Add a virtual `'chatbot'` registry entry so GUI config GET/POST still resolves the chatbot appId after the real app module is deleted (REQ-CONV-013).
