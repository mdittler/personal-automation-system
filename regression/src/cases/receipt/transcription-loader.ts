 ...
 */
function readCapped(path: string, max: number): { content: string; truncated: boolean } {
...
function validateLineItem(raw: unknown, idx: number, path: string): TranscriptionLineItem {
	...
	if (typeof obj.name !== 'string' || obj.name.trim().length === 0) {
		throw new TranscriptionLoadError(
			`lineItems[${idx}].name missing or not a non-empty string`,
			path,
		);
	}
	if (!isFiniteNumber(obj.totalPrice)) {
		throw new TranscriptionLoadError(
			`lineItems[${idx}].totalPrice missing or not a finite number`,
			path,
		);
	}
...
export function loadTranscription(yamlPath: string): ReceiptTranscription {
	...
	if (yamlPath.split(/[/\\]/).includes('..')) {
		throw new TranscriptionLoadError('transcription path contains traversal segments', yamlPath);
	}
	...
	if (root.subtotal !== undefined && !isNonNegFinite(root.subtotal)) {
		throw new TranscriptionLoadError(
			'subtotal must be a non-negative finite number if present',
			yamlPath,
		);
	}
	if (root.tax !== undefined && !isNonNegFinite(root.tax)) {
		throw new TranscriptionLoadError(
			'tax must be a non-negative finite number if present',
			yamlPath,
		);
	}
