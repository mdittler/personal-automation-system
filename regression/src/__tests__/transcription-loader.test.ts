...
import {
	...
	loadTranscription,
} from '../cases/receipt/transcription-loader.js';

function tempFixture(
	yaml: string,
	options?: { withSha?: 'matching' | 'mismatching' | 'missing' },
): string {
...
describe('loadTranscription — validation failures', () => {
	it('throws on missing total', () => {
		expect(() =>
			loadTranscription(tempFixture('lineItems:\n  - name: A\n    totalPrice: 1\n')),
		).toThrow(TranscriptionLoadError);
	});

	it('throws on negative top-level total', () => {
		expect(() =>
			loadTranscription(tempFixture('total: -1\nlineItems:\n  - name: A\n    totalPrice: 1\n')),
	...
	it('throws on missing line-item name', () => {
		expect(() =>
			loadTranscription(tempFixture('total: 5\nlineItems:\n  - totalPrice: 5\n')),
		).toThrow(/name/);
	});
	...
	it('throws on wrong-type optional date (number)', () => {
		expect(() =>
			loadTranscription(
				tempFixture('date: 20260515\ntotal: 5\nlineItems:\n  - name: A\n    totalPrice: 5\n'),
			),
		).toThrow(/date/);
	});

	it('throws on malformed YAML', () => {
		expect(() => loadTranscription(tempFixture('not: valid: yaml: at all: ::'))).toThrow(
			TranscriptionLoadError,
		);
	});
