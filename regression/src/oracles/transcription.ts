...
function lineSatisfies(
	p: ReceiptLineItem,
	t: TranscriptionLineItem,
...
export function runTranscriptionOracle(
	...
	if (!Number.isFinite(parsed.total))
		return { verdict: VERDICT.error, details: `parsed.total not finite (${parsed.total})` };
	for (const li of parsed.lineItems) {
		if (!Number.isFinite(li.totalPrice))
			return { verdict: VERDICT.error, details: `parsed totalPrice not finite for ${li.name}` };
		if (!Number.isFinite(li.quantity))
			return { verdict: VERDICT.error, details: `parsed quantity not finite for ${li.name}` };
		if (li.unitPrice !== null && !Number.isFinite(li.unitPrice))
			return { verdict: VERDICT.error, details: `parsed unitPrice not finite for ${li.name}` };
	}
	...
	for (const pli of unmatchedParsed) {
		...
		const near = transcription.lineItems.find(
			(t) =>
				normalizeName(t.name) === normalizeName(pli.name) &&
				priceCents(t.totalPrice) === priceCents(pli.totalPrice),
		);
		if (near) {
			const probe = lineSatisfies(pli, near);
			failures.push(
				`${probe.reason ?? 'mismatch'} mismatch for ${pli.name}: parsed q=${pli.quantity} u=${pli.unitPrice} t=${pli.totalPrice}`,
			);
		} else {
	...
	for (const t of trxRemaining) {
		if (t.confidence === 'high') {
			failures.push(
				`missing high-confidence transcription item: ${t.name} @ ${t.totalPrice.toFixed(2)}`,
			);
		}
	...
	if (transcription.subtotal !== undefined && parsed.subtotal !== null) {
		if (!withinMoney(parsed.subtotal, transcription.subtotal)) {
			failures.push(
				`subtotal mismatch: parsed=${parsed.subtotal} expected=${transcription.subtotal}`,
			);
		}
	}
